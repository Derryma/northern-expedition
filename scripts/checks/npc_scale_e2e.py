# -*- coding: utf-8 -*-
"""15.11 唐繼堯統治動搖：在真前端跑完整條路，看戰力點真的被砍了沒。
...
def start_server():
    ...
    proc = subprocess.Popen(['python3', '-c', 'from backend.server import run; run(port=8774)'], env=env,
...
// ---- 1. 地圖上找得到劉湘的部隊嗎 ----
const liu = d.allArmies().find(a => a.generalId === 'tang_jiyao');
if (!liu) return { __錯誤__: '地圖上沒有唐繼堯的部隊' };
out.部隊 = { id: liu.id, 番號: liu.designator, 將領: liu.general };
out.補兵前 = { ...d.armyUnits(liu) };
out.戰力前 = d.forcePoints(d.armyUnits(liu));
// 對照組：這張卡沒點名的 NPC 部隊。它若也變多，那是每回合例行補兵，不是這張卡。
out.對照組補兵前 = Object.fromEntries(d.allArmies()
  .filter(a => ['long_yun','yang_sen','feng_yuxiang'].includes(a.generalId))
...
snap.event_pool = ['tang_jiyao_shaken'];
snap.turn = 2;
...
out.戰力後 = liu ? d.forcePoints(d.armyUnits(liu)) : null;
