...
FRONTEND_SOURCE = (REPO_ROOT / "frontend" / "app.js").read_text(encoding="utf-8")
EVENT_CARDS_PATH = REPO_ROOT / "cards" / "data" / "event_cards.json"
FUNCTION_CARDS_PATH = REPO_ROOT / "cards" / "data" / "function_cards.json"
...
class BackendTests(unittest.TestCase):
    ...
    def test_affiliation_slot_upgrade_returns_slot_delta(self):
        ...
        self.assertEqual(result["affiliation_slot_delta"],
                         {"owner": "N", "general_id": "he_yingqin", "amount": 1, "cap": 3})
    ...
    def test_intel_and_police_cards_create_timed_effects(self):
        ...
        # 卡片 id 還是 police_system，但 mechanic 與 kind 已改名為 counter_intel——
        # 「警政單位」是另一張卡（擋黑幫暴動），先前兩者的命名交叉，
        # 卡面文字也因此寫錯了對象。
        self.assertTrue(any(effect["kind"] == "counter_intel" and effect["remaining_turns"] == 3 for effect in effects))
    ...
    def test_sixty_six_cards_carry_a_story(self):
        ...
        # 再 +1：票號金融網 → 70；紅槍會起義 → 71。
        self.assertEqual(len(with_story), 71)
...
class EventCardTests(unittest.TestCase):
    ...
    def test_cycle_fires_every_third_turn_and_holds_the_economy(self):
        ...
        # 回應名單要對得上這張卡的 scope，而不是對上「上次跑這條測試時
        # 剛好抽到哪一張」。先前這裡寫死 [drawer]，於是新增一張功能卡、
        # 洗牌的亂數流一動，抽到的換成 all_players 的卡就紅了——
        # 那不是回歸，是測試綁錯了東西。
        if view["needs_every_faction"]:
            expected = engine._event_eligible_players(
                engine._event_template(view["card"]["id"]))
            self.assertEqual(sorted(view["responders"]), sorted(expected))
...
class CardBatchTests(unittest.TestCase):
    ...
    def test_police_precinct_blocks_and_quells_gang_riots(self):
        ...
        # 別的省不受影響。河南已經從這張卡的名單裡拿掉（改歸紅槍會起義管），
        # 而直系在這張卡的四個省裡只握有湖北——湖北正是被護盾罩住的那個。
        # 所以改打五省聯軍的江蘇：護盾是「誰的哪一省」，不是整張卡失效。
        engine.use_function("N", "du_yuesheng_gamble", target_owner="S", target_province="江蘇")
...
class FrontendBackendParityTests(unittest.TestCase):
    ...
    def test_the_frontend_owns_no_copy_of_the_defection_formula(self):
        """規則只能有一份。前端先前有一份逐字相同的副本，還靠這一組測試盯著兩邊——
        那不是單一來源，那是兩份規則加一個看門的。現在前端只顯示後端的報價。
        """
        for fragment in ("0.45 - ", "* 1.25", "Math.min(0.60", "* 3 + ", "defectionResistance",
                         "DEFECTION_RESISTANCE_TRAITS"):
            self.assertNotIn(fragment, FRONTEND_SOURCE,
                             f"前端又出現策反公式的一部分：{fragment}")
        self.assertIn("/api/defection-quote", FRONTEND_SOURCE, "面板要跟後端要報價")

    def test_the_client_cannot_dictate_the_odds(self):
        """帶 general_id 時，客戶端送什麼忠誠／戰力／抗性都不算數。"""
        engine = GameEngine(seed=3)
        engine.new_game()
        tactical = {
            "armies": {"W-1": {"generalId": "wu_peifu", "faction": "W", "status": "active",
                               "units": {"infantry": 10, "cavalry": 2}}},
            "generalTrees": {"W": {"generals": {"wu_peifu": {"id": "wu_peifu", "loyalty": 5,
                                                             "traits": []}}}},
            "generalOwners": {"wu_peifu": "W"}, "loyaltyOverrides": {},
        }
        engine.state["players"]["F"]["treasury"] = 99999
        honest = engine.defection_quote("wu_peifu", tactical)
        cheated = engine.attempt_defection_with_force(
            "F", loyalty=1, force=1, resistance=-99, general_id="wu_peifu", tactical=tactical)
        self.assertEqual(cheated["cost"], honest["cost"], "成本被客戶端帶偏了")
        self.assertAlmostEqual(cheated["chance"], honest["chance"], places=9,
                               msg="成功率被客戶端帶偏了")

    def test_the_resistance_table_lives_in_the_backend(self):
        engine = GameEngine(seed=3)
        self.assertEqual(engine.DEFECTION_RESISTANCE_TRAITS, {"buddhist_general": 0.05})
        tactical = {
            "armies": {"H-1": {"generalId": "tang_shengzhi", "faction": "H", "status": "active",
                               "units": {"infantry": 10}}},
            "generalTrees": {"H": {"generals": {
                "tang_shengzhi": {"id": "tang_shengzhi", "loyalty": 5,
                                  "traits": ["buddhist_general"]}}}},
            "generalOwners": {"tang_shengzhi": "H"}, "loyaltyOverrides": {},
        }
        quote = engine.defection_quote("tang_shengzhi", tactical)
        self.assertAlmostEqual(quote["resistance"], 0.05)
        plain = dict(tactical)
        plain["generalTrees"] = {"H": {"generals": {
            "tang_shengzhi": {"id": "tang_shengzhi", "loyalty": 5, "traits": []}}}}
        self.assertGreater(engine.defection_quote("tang_shengzhi", plain)["chance"],
                           quote["chance"], "〈佛教將軍〉要真的壓低成功率")
...
class LoyaltyRulesTests(unittest.TestCase):
    ...
    def test_the_frontend_has_no_loyalty_formula_at_all(self):
        """守門：忠誠只有一個計算引擎。前端拿不到後端的值時顯示「計算中」，不自己算。"""
        body = FRONTEND_SOURCE[FRONTEND_SOURCE.index("function calculateGeneralLoyalty"):]
        body = body[:body.index("\n// 後端算完的忠誠")]
        self.assertIn("backendLoyalty?.[general.id]", body)
        for fragment in ("relationPenalty", "relativePower", "battleLoss", "baseLoyalty",
                         "LOYALTY_BASELINE_ARMY_UNITS", "averageForce"):
            self.assertNotIn(fragment, body, f"前端又自己算忠誠了：{fragment}")
        self.assertIn("pending: true", body, "沒有後端的值就要顯示計算中")
        self.assertNotIn("function traitLoyaltyAdjustment", FRONTEND_SOURCE,
                         "列強關係扣分是後端 compute_loyalty 的事")
        self.assertIn('"loyalty": ENGINE.loyalty_report',
                      (pathlib.Path(__file__).resolve().parent / "server.py")
                      .read_text(encoding="utf-8"),
                      "server 沒把忠誠算給前端")

    def test_the_panel_refreshes_instead_of_showing_a_stale_number(self):
        """守門：本地快照與已發布的那份不一致時，畫面要主動補發一次再重畫。"""
        self.assertIn("function ensureBackendDerivedFresh", FRONTEND_SOURCE)
        self.assertIn("function backendDerivedStateIsStale", FRONTEND_SOURCE)
        panel = FRONTEND_SOURCE[FRONTEND_SOURCE.index("function renderPanel(panelName)"):]
        panel = panel[:panel.index("switch (panelName)")]
        self.assertIn("ensureBackendDerivedFresh(panelName)", panel,
                      "面板重畫前沒有確認後端算的數字是不是舊的")
        self.assertIn("function requestLoyaltyRefresh", FRONTEND_SOURCE)

    def test_the_defection_request_sends_no_numbers_of_its_own(self):
        """策反請求只指名對象；忠誠、戰力、抗性由後端從共享快照自己算。"""
        body = FRONTEND_SOURCE[FRONTEND_SOURCE.index("/api/attempt-defection") - 900:]
        body = body[:body.index("/api/attempt-defection") + 400]
        self.assertIn("general_id: general?.id", body)
        for field in ("loyalty,", "force: forcePoints", "resistance:"):
            self.assertNotIn(field, body, f"前端還在送自己算的 {field}")
...
class NpcStructuralChangeTests(unittest.TestCase):
    ...
    def test_the_absorbed_army_is_emptied_and_retired(self):
        ...
        # 退場狀態必須是前後端都認得的那幾個之一，否則空殼部隊會留在地圖上。
        self.assertEqual(qian["status"], "destroyed")
        self.assertIn(qian["status"], GameEngine.DEAD_ARMY_STATUSES)
...
class FrontendEffectChannelTests(unittest.TestCase):
    """後端交辦給前端的每一種 kind，前端都要有人收。

    這條通道是靜默失效的重災區：後端 append 一筆 `pending_frontend_effects`，
    前端沒登記處理器的話只會在主控台 warn 一聲，然後照樣銷帳——
    錢扣了、贏家抽出來了，畫面上什麼都沒發生，而且測試全綠。

    `npc_general_recruited` 就是這樣漏掉的：付費招募做完的那一批，
    後端掛了交辦，前端沒有對應的處理器，將領根本沒換手。
    """

    ENGINE_SOURCES = ("card_engine.py", "foreign_punishment.py")
    ...
    def _handler_kinds(self) -> set:
        import re
        app = (pathlib.Path(__file__).resolve().parents[1]
               / "frontend" / "app.js").read_text(encoding="utf-8")
        block = app.split("const PENDING_EFFECT_HANDLERS = {", 1)[1]
        block = block.split("\nasync function consumePendingFrontendEffects", 1)[0]
        # 有些處理器要 await 後端（忠誠加減），寫成 `kind: async (…) =>`。
        return set(re.findall(r'^  ([a-z_]+): (?:async )?\(', block, re.M))

    def test_every_kind_the_backend_queues_has_a_frontend_handler(self):
        queued = self._queued_kinds()
        self.assertTrue(queued, "抓不到任何交辦 kind，這條測試自己壞了")
        missing = sorted(queued - self._handler_kinds())
        self.assertEqual(missing, [],
                         f"後端會掛這些交辦，前端卻沒人收：{missing}")

    def test_the_generic_frontend_effects_channel_is_covered_too(self):
        """卡片資料可以用 `frontend_effects` 直接指定 kind——那些也要有人收。"""
        data = load_game_data()
        handlers = self._handler_kinds()
        seen = set()

        def walk(block):
            if not isinstance(block, dict):
                return
            for entry in (block.get("frontend_effects") or []):
                seen.add(str(entry.get("kind")))
        ...
        self.assertTrue(seen, "沒有任何卡用 frontend_effects，這條測試自己壞了")
        self.assertEqual(sorted(seen - handlers), [],
                         f"卡片指定了沒人收的 kind：{sorted(seen - handlers)}")

    def test_balfour_goes_through_the_backend_channel(self):
        """2.6 的『隨機一位將領 +1』要和 14.9 走同一條路。

        先前它在 applyFrontendEventEffects() 裡自己挑人、自己加忠誠——
        那是同一條規則的第二份實作，而且用的是沒有種子的 Math.random()，
        多人連線時每個 client 會挑到不同的人。
        """
        engine = GameEngine(seed=3)
        card = engine._event_template("balfour_declaration")
        spec = (card["apply"]["frontend_effects"] or [])[0]
        self.assertEqual((spec["kind"], spec["count"], spec["amount"]),
                         ("loyalty_random", 1, 1))
        app = (pathlib.Path(__file__).resolve().parents[1]
               / "frontend" / "app.js").read_text(encoding="utf-8")
        body = app.split("function applyFrontendEventEffects(", 1)[1].split("\nfunction ", 1)[0]
        self.assertNotIn("balfour_declaration", body,
                         "貝爾福宣言不該再有前端特例")

    def test_it_really_queues_one_hit_per_player(self):
        engine = GameEngine(seed=3)
        engine.new_game()
        card = engine._event_template("balfour_declaration")
        engine._apply_event_payload(card["apply"], players=None, card=card)
        for code in engine.state["players"]:
            hits = [e for e in engine.state["players"][code]["pending_frontend_effects"]
                    if e["kind"] == "loyalty_random"]
            self.assertEqual(len(hits), 1, code)
            self.assertEqual((hits[0]["count"], hits[0]["amount"]), (1, 1))


class RedSpearUprisingTests(unittest.TestCase):
    """〈紅槍會起義〉：與〈杜月笙的豪賭〉、〈洪門起義〉同一套機制，換一組省份。

    機制本身是共用的（`qing_gang_riot`），所以這裡驗的是這張卡有沒有真的接上，
    以及三張同機制的卡有沒有被寫成互相打架。
    """

    CARD = "red_spear_uprising"
    SIBLINGS = ("du_yuesheng_gamble", "hongmen_uprising", "red_spear_uprising")
    ...
    def test_the_card_exists_with_two_copies(self):
        from backend.card_engine import FUNCTION_CARD_COPIES
        engine = self._engine()
        card = engine._card_template(self.CARD)
        self.assertEqual(card["name"], "紅槍會起義")
        self.assertEqual(FUNCTION_CARD_COPIES[self.CARD], 2)

    def test_it_covers_the_five_northern_provinces(self):
        engine = self._engine()
        card = engine._card_template(self.CARD)
        self.assertEqual(card["provinces"], ["直隸", "山東", "山西", "河南", "奉天"])
        # 卡片上的省份必須真的在地圖上，否則這張卡永遠指不到目標。
        on_map = {c["province"] for c in engine.data["strategic_map"]["cities"]}
        for province in card["provinces"]:
            self.assertIn(province, on_map)

    def test_the_three_uprising_cards_do_not_overlap(self):
        """三張同機制的卡各管一組省份，一個省只歸一張卡管。

        河南一度同時掛在杜月笙的豪賭與紅槍會起義底下，後來把它從杜月笙那張
        拿掉了——同一個省有兩張卡指得到，等於同一件事有兩條路。
        """
        engine = self._engine()
        seen = {}
        for card_id in self.SIBLINGS:
            provinces = engine._card_template(card_id)["provinces"]
            self.assertTrue(provinces, card_id)
            self.assertEqual(len(provinces), len(set(provinces)),
                             f"{card_id} 自己的省份清單有重複")
            for province in provinces:
                self.assertNotIn(province, seen,
                                 f"{province} 同時出現在 {seen.get(province)} 與 {card_id}")
                seen[province] = card_id

    def test_each_cards_effect_text_lists_exactly_its_own_provinces(self):
        """效果文字與 provinces 欄位要對得上——改了一邊沒改另一邊，
        玩家看到的說明就會和實際擋下來的不一樣。"""
        engine = self._engine()
        for card_id in self.SIBLINGS:
            card = engine._card_template(card_id)
            for province in card["provinces"]:
                self.assertIn(province, card["effect"],
                              f'{card_id} 的效果文字沒提到 {province}')
            others = {p for other in self.SIBLINGS if other != card_id
                      for p in engine._card_template(other)["provinces"]}
            for province in others:
                self.assertNotIn(province, card["effect"],
                                 f'{card_id} 的效果文字提到了別張卡的 {province}')

    def test_a_province_already_rioting_cannot_be_hit_again(self):
        """同一個省不准疊第二次暴動——疊了會憑空生錢。

        產出只會歸零一次（本來就沒得再少），但分潤是逐筆結算的：
        兩個發動者會各拿一份「被截斷的一半」，目標少 11、兩人合拿 24。
        每張卡本來就有 2 份，所以兩個玩家各打一張同名卡就撞得到，
        不必靠兩張不同的卡。
        """
        engine = self._engine()
        for city in engine.data["strategic_map"]["cities"]:
            if city["province"] == "山東":
                engine.state["city_owners"][city["id"]] = "W"
        engine._refresh_city_income()
        engine.state["players"]["F"]["hand"] = [self.CARD]
        engine.use_function("F", self.CARD, target_owner="W", target_province="山東")
        engine.state["players"]["S"]["hand"] = [self.CARD]
        with self.assertRaises(ValueError):
            engine.use_function("S", self.CARD,
                                target_owner="W", target_province="山東")
        self.assertEqual(len(engine.state["city_output_effects"]), 1)
        payouts = engine._qing_gang_riot_rewards()
        self.assertEqual([r["initiator"] for r in payouts], ["F"],
                         "只有一個發動者拿得到分潤")

    def test_the_same_card_can_still_hit_a_different_province(self):
        """擋的是同一個省疊兩次，不是把整張卡鎖死。"""
        engine = self._engine()
        for city in engine.data["strategic_map"]["cities"]:
            if city["province"] in ("河南", "山東"):
                engine.state["city_owners"][city["id"]] = "W"
        engine._refresh_city_income()
        engine.state["players"]["F"]["hand"] = [self.CARD, self.CARD]
        engine.use_function("F", self.CARD, target_owner="W", target_province="河南")
        engine.use_function("F", self.CARD, target_owner="W", target_province="山東")
        self.assertEqual(len(engine.state["city_output_effects"]), 2)

    def test_two_different_targets_in_the_same_province_are_still_allowed(self):
        """擋的是「同一個省、同一個目標」。同省不同家的城市各自暴動不衝突。"""
        engine = self._engine()
        cities = [c["id"] for c in engine.data["strategic_map"]["cities"]
                  if c["province"] == "山東"]
        engine.state["city_owners"][cities[0]] = "W"
        engine.state["city_owners"][cities[1]] = "S"
        engine._refresh_city_income()
        engine.state["players"]["F"]["hand"] = [self.CARD, self.CARD]
        engine.use_function("F", self.CARD, target_owner="W", target_province="山東")
        engine.use_function("F", self.CARD, target_owner="S", target_province="山東")
        self.assertEqual(len(engine.state["city_output_effects"]), 2)

    def test_the_handwritten_readme_table_lists_the_same_provinces(self):
        """`cards/README.md` 上半部那張功能卡表是**手寫**的，不是產生的。

        它等於第二份真源：這次把河南從杜月笙的豪賭拿掉、又新增一張卡，
        那張表兩處都會過時而沒有任何測試會紅。這條就是那個把關。
        （STORY-TABLE 與 EVENT-TABLE 兩段有自己的同步測試，這裡不重複。）
        """
        readme = (pathlib.Path(__file__).resolve().parents[1]
                  / "cards" / "README.md").read_text(encoding="utf-8")
        table = readme.split("<!-- STORY-TABLE:BEGIN -->", 1)[0]
        engine = self._engine()
        for card_id in self.SIBLINGS:
            card = engine._card_template(card_id)
            row = next((line for line in table.splitlines()
                        if line.startswith(f'| {card["name"]} |')), None)
            self.assertIsNotNone(row, f'手寫表裡沒有 {card["name"]}')
            if "同上" in row:
                # 「同上，範圍為……」的寫法只列省份，不重述機制。
                for province in card["provinces"]:
                    self.assertIn(province, row, f'{card["name"]} 少列了 {province}')
                continue
            for province in card["provinces"]:
                self.assertIn(province, row, f'{card["name"]} 少列了 {province}')
            others = {p for other in self.SIBLINGS if other != card_id
                      for p in engine._card_template(other)["provinces"]}
            for province in others:
                self.assertNotIn(province, row,
                                 f'{card["name"]} 那一列列到了別張卡的 {province}')

    def test_it_really_zeroes_the_targets_cities_in_that_province(self):
        engine = self._engine()
        engine._refresh_city_income()
        # 直隸三座城都給直系，才有東西可以打
        hebei = [c["id"] for c in engine.data["strategic_map"]["cities"]
                 if c["province"] == "直隸"]
        for city_id in hebei:
            engine.state["city_owners"][city_id] = "W"
        engine._refresh_city_income()
        before = int(engine.state["players"]["W"]["income"])
        engine.state["players"]["F"]["hand"] = [self.CARD]
        engine.use_function("F", self.CARD, target_owner="W", target_province="直隸")
        engine._refresh_city_income()
        self.assertLess(int(engine.state["players"]["W"]["income"]), before)
        effect = next(e for e in engine.state["city_output_effects"]
                      if e.get("card_id") == self.CARD)
        self.assertEqual(effect["province"], "直隸")
    ...
    def test_a_province_outside_the_list_is_refused(self):
        ...
        engine.state["players"]["F"]["hand"] = [self.CARD]
        with self.assertRaises(ValueError):
            engine.use_function("F", self.CARD, target_owner="W", target_province="江蘇")

    def test_half_the_severed_output_goes_to_the_initiator(self):
        """被截斷的現金與工廠，每回合有一半轉給發動者。"""
        engine = self._engine()
        for city in engine.data["strategic_map"]["cities"]:
            if city["province"] == "山東":
                engine.state["city_owners"][city["id"]] = "W"
        ...
        engine.state["players"]["F"]["hand"] = [self.CARD]
        engine.use_function("F", self.CARD, target_owner="W", target_province="山東")
        rewards = engine._qing_gang_riot_rewards()
        mine = [r for r in rewards if r.get("initiator") == "F"]
        self.assertTrue(mine, f"發動者沒有分到錢：{rewards}")
        self.assertGreater(mine[0]["cash"] + mine[0]["factory"], 0)


class SecurityChoiceBranchTests(unittest.TestCase):
    """十四、治安事件裡兩張要表態的卡，兩邊分支都要真的落地。

    `ChoiceCardBothBranchesTests` 已經掃過全部表態卡「有沒有改到狀態」，
    但那是整包指紋比對——改到什麼它不管。這裡逐項盯**改對了沒有**。
    """
    ...
    def _apply(self, engine, card_id, option_id, player="F"):
        card = engine._event_template(card_id)
        option = next(o for o in card["resolution"]["options"] if o["id"] == option_id)
        applied = engine._apply_event_payload(option.get("apply") or {},
    ...
    def test_suppressing_halts_the_four_cities_and_extends_the_student_movement(self):
        engine = self._engine()
        before = int(engine.state["players"]["F"]["income"])
        kinds = {i["kind"] for i in self._apply(engine, "anti_imperialist_march", "suppress")}
        self.assertEqual(kinds, {"city_halt", "event_duration_bonus"})
        self.assertLess(int(engine.state["players"]["F"]["income"]), before,
                        "出兵鎮壓要讓四座城產出打折")
        halt = next(e for e in engine.state["city_output_effects"]
                    if e.get("card_id") == "anti_imperialist_march")
        self.assertEqual(sorted(halt["city_ids"]),
                         sorted(["guangzhou", "hankou", "shanghai", "tianjin"]))

    def test_tolerating_costs_relations_with_four_powers_and_nothing_else(self):
        engine = self._engine()
        before = dict(engine.state["players"]["F"]["foreign_relations"])
        income_before = int(engine.state["players"]["F"]["income"])
        self._apply(engine, "anti_imperialist_march", "tolerate")
        after = engine.state["players"]["F"]["foreign_relations"]
        for power in ("uk", "us", "fr", "jp"):
            self.assertEqual(after[power], before[power] - 2, power)
        self.assertEqual(int(engine.state["players"]["F"]["income"]), income_before,
                         "放任不該動到城市產出")
        self.assertEqual(engine.state["city_output_effects"], [])

    def test_the_two_branches_of_the_march_are_not_the_same_thing(self):
        one, two = self._engine(), self._engine()
        self._apply(one, "anti_imperialist_march", "suppress")
        self._apply(two, "anti_imperialist_march", "tolerate")
        self.assertNotEqual(
            (int(one.state["players"]["F"]["income"]),
             one.state["players"]["F"]["foreign_relations"]["uk"]),
            (int(two.state["players"]["F"]["income"]),
             two.state["players"]["F"]["foreign_relations"]["uk"]))

    # ---- 14.9 地方官貪腐 ----

    def test_reforming_pays_off_later_and_costs_loyalty_now(self):
        engine = self._engine()
        before = int(engine.state["players"]["F"]["income"])
        kinds = {i["kind"] for i in self._apply(engine, "local_official_graft", "reform")}
        self.assertEqual(kinds, {"graft_state", "delayed_output", "frontend_effect"})
        self.assertGreater(int(engine.state["players"]["F"]["income"]), before)
        hits = [e for e in engine.state["players"]["F"]["pending_frontend_effects"]
                if e["kind"] == "loyalty_random"]
        self.assertEqual(len(hits), 1)
        self.assertEqual((hits[0]["count"], hits[0]["amount"]), (2, -1),
                         "整頓要得罪兩個人，各 −1")

    def test_neglecting_costs_income_every_turn_and_stacks(self):
        engine = self._engine()
        before = int(engine.state["players"]["F"]["income"])
        self._apply(engine, "local_official_graft", "neglect")
        once = int(engine.state["players"]["F"]["income"])
        self.assertLess(once, before, "放任要每回合扣錢")
        # 再放任一次要疊一層，不是覆蓋
        self._apply(engine, "local_official_graft", "neglect")
        self.assertLess(int(engine.state["players"]["F"]["income"]), once,
                        "放任第二次要再疊一層")

    def test_reforming_clears_what_neglecting_piled_up(self):
        engine = self._engine()
        clean = int(engine.state["players"]["F"]["income"])
        self._apply(engine, "local_official_graft", "neglect")
        self._apply(engine, "local_official_graft", "neglect")
        self.assertLess(int(engine.state["players"]["F"]["income"]), clean)
        self._apply(engine, "local_official_graft", "reform")
        # 整頓清掉貪腐層之後還會加上 5 回合的 $+3，所以只驗「回到乾淨以上」。
        self.assertGreaterEqual(int(engine.state["players"]["F"]["income"]), clean)

    # ---- 兩張卡都要真的走得到玩家按下去的那條路 ----

    def test_both_cards_are_reachable_through_respond_event(self):
        for card_id in ("anti_imperialist_march", "local_official_graft"):
            for option in (GameEngine(seed=3)._event_template(card_id)
                           ["resolution"]["options"]):
                engine = GameEngine(seed=3)
                engine.state["turn"] = 2
                engine.state["event_pool"] = [card_id]
                ...
                view = engine.pending_event_view()
                self.assertEqual(view["card"]["id"], card_id)
                outcome = engine.respond_event(view["waiting_for"], choice=option["id"])
                self.assertTrue(outcome["applied"],
                                f'{card_id}／{option["id"]} 按下去什麼都沒發生')


class MobilOilSupplyTests(unittest.TestCase):
    """〈美孚石油供應〉：免疫〈國際油價上漲〉的建軍加價。

    這張卡曾經是**完全空轉**的：`use_function` 掛上一個
    `oil_price_immunity` 的限時效果，然後**沒有任何地方讀它**——
    打出去之後成本一分錢都沒少。旗標存在不等於機制存在。
    """

    def _engine(self, with_card=True, player="F"):
        engine = GameEngine(seed=3)
        engine.new_game()
        if with_card:
            engine.state["players"][player]["foreign_relations"]["us"] = 6
            engine.state["players"][player]["hand"] = ["us_socony_oil"]
            engine.use_function(player, "us_socony_oil")
        return engine

    def _surge(self, engine):
        card = engine._event_template("world_oil_price_surge")
        engine._apply_event_payload(card["apply"], players=None, card=card)

    def test_the_card_declares_which_card_it_is_immune_to(self):
        """免疫哪張卡是**卡片資料**說了算，不是寫死在引擎裡。"""
        engine = GameEngine(seed=3)
        card = engine._card_template("us_socony_oil")
        self.assertEqual(card["immune_cards"], ["world_oil_price_surge"])
        self.assertEqual(card["duration_turns"], 10)

    def test_without_the_card_the_surge_really_costs_more(self):
        engine = self._engine(with_card=False)
        base = engine._unit_cost_for("F", "infantry")
        self._surge(engine)
        self.assertGreater(engine._unit_cost_for("F", "infantry")[0], base[0])

    def test_with_the_card_the_ground_surcharge_is_gone(self):
        plain = GameEngine(seed=3); plain.new_game()
        base = plain._unit_cost_for("F", "infantry")
        engine = self._engine()
        self._surge(engine)
        self.assertEqual(engine._unit_cost_for("F", "infantry"), base,
                         "有穩定油源就不該吃到加價")

    def test_the_navy_surcharge_is_covered_too(self):
        """卡片寫的是免疫〈國際油價上漲〉，那張卡的水軍加價也是它造成的。"""
        engine = self._engine()
        self._surge(engine)
        self.assertEqual(engine._production_multiplier("F", "navy"),
                         {"cash": 1.0, "factory": 1.0})

    def test_only_the_holder_is_immune(self):
        engine = self._engine()
        self._surge(engine)
        plain = GameEngine(seed=3); plain.new_game()
        self.assertEqual(engine._unit_cost_for("W", "infantry"),
                         plain._unit_cost_for("W", "infantry")[0:1]
                         + engine._unit_cost_for("W", "infantry")[1:2]
                         if False else engine._unit_cost_for("W", "infantry"))
        self.assertGreater(engine._unit_cost_for("W", "infantry")[0],
                           plain._unit_cost_for("W", "infantry")[0],
                           "沒拿卡的人照吃加價")

    def test_it_does_not_block_other_cards_surcharges(self):
        """免疫只認名單上那張卡。英國軍火出口管制照樣加價。"""
        engine = self._engine()
        engine.state.setdefault("production_cost_multipliers", []).append({
            "id": "britain_arms_export_control:1", "card_id": "britain_arms_export_control",
            "label": "英國軍火出口管制", "arms": ["ground"],
            "cash": 1.3, "factory": 1.3, "until_turn": 99, "players": None})
        plain = GameEngine(seed=3); plain.new_game()
        self.assertGreater(engine._unit_cost_for("F", "infantry")[0],
                           plain._unit_cost_for("F", "infantry")[0])

    def test_the_immunity_does_not_swallow_a_discount(self):
        """香港軍火交易那種降價不該被免疫吃掉——免疫擋的是加價。"""
        engine = self._engine()
        engine.state.setdefault("production_cost_multipliers", []).append({
            "id": "world_oil_price_surge:1", "card_id": "world_oil_price_surge",
            "label": "假設的降價", "arms": ["ground"],
            "cash": 0.7, "factory": 0.7, "until_turn": 99, "players": None})
        self.assertLess(engine._production_multiplier("F", "ground")["cash"], 1.0)

    def test_the_immunity_expires_with_the_effect(self):
        engine = self._engine()
        effect = next(e for e in engine.state["players"]["F"]["timed_effects"]
                      if e["kind"] == "oil_price_immunity")
...
class DeadMechanismAuditTests(unittest.TestCase):
    """空轉機制稽核：這一批全是「旗標／欄位寫進去了，卻沒有任何人讀」的洞。

    共同的失效樣態是同一個：資料檔上白紙黑字寫著一條規則，引擎卻沒有
    對應的讀取點，於是規則退化成「玩家自主遵守」。每一條都用**執行**驗，
    不看程式碼。
    """
    ...
    @staticmethod
    def _fire(engine, card_id, *, tactical=None, choice=None):
        engine.state["event_pool"] = [card_id]
        current = int(engine.state["turn"])
        engine.state["turn"] = ((current // 3) + 1) * 3 - 1
        engine.next_turn(active_player="F", tactical=tactical)
        view = engine.pending_event_view()
        while view:
    ...
    # ---- A-1 非戰公約：blocks_declaration ----

    def _sign_the_pact(self, choice):
        engine = GameEngine(seed=5)
        self._fire(engine, "kellogg_briand_pact", choice=choice)
        return engine

    def test_signing_the_kellogg_pact_really_blocks_declaring_war(self):
        """卡面寫「不得宣戰」，就不能只靠玩家自己記得。"""
        engine = self._sign_the_pact("sign")
        peace = engine.active_timed_flag("F", "forced_peace")
        self.assertTrue(peace and peace.get("blocks_declaration"))
        with self.assertRaisesRegex(ValueError, "不得宣戰"):
            engine.set_diplomacy("F", "W", "war")
        self.assertEqual(engine.state["players"]["F"]["warlord_relations"]["W"]["status"], "peace")

    def test_ignoring_the_pact_leaves_you_free_to_declare(self):
        engine = self._sign_the_pact("ignore")
        self.assertIsNone(engine.active_timed_flag("F", "forced_peace"))
        engine.set_diplomacy("F", "W", "war")
        self.assertEqual(engine.state["players"]["F"]["warlord_relations"]["W"]["status"], "war")

    def test_an_expired_ceasefire_stops_blocking(self):
        """突變測試逃掉過一次：active_timed_flag 不看 remaining_turns 也沒人紅。

        旗標倒數完就不該再擋——這條測試守的正是那個到期判斷。
        """
        engine = self._sign_the_pact("sign")
        flag = engine.active_timed_flag("F", "forced_peace")
        self.assertIsNotNone(flag)
        flag["remaining_turns"] = 0
        self.assertIsNone(engine.active_timed_flag("F", "forced_peace"),
                          "倒數歸零的旗標不該還被當成生效中")
        engine.set_diplomacy("F", "W", "war")
        self.assertEqual(engine.state["players"]["F"]["warlord_relations"]["W"]["status"], "war")

    def test_a_permanent_flag_never_expires(self):
        """remaining_turns 為 None 是「無期限」，不是「已經到期」。"""
        engine = GameEngine(seed=5)
        engine.state["players"]["F"]["timed_effects"].append(
            {"kind": "forced_peace", "name": "測試用永久停戰",
             "remaining_turns": None, "permanent": True, "blocks_declaration": True})
        self.assertIsNotNone(engine.active_timed_flag("F", "forced_peace"))
        with self.assertRaisesRegex(ValueError, "不得宣戰"):
            engine.set_diplomacy("F", "W", "war")

    def test_making_peace_is_never_blocked_by_the_pact(self):
        """擋的只有宣戰。已經在打的要議和，強制和平不該卡住。"""
        engine = GameEngine(seed=5)
        engine.set_diplomacy("F", "W", "war")
        self._fire(engine, "kellogg_briand_pact", choice="sign")
        engine.state["players"]["F"]["warlord_relations"]["W"]["war_started_turn"] = \
            int(engine.state["turn"]) - 10
        engine.set_diplomacy("F", "W", "peace")
        self.assertEqual(engine.state["players"]["F"]["warlord_relations"]["W"]["status"], "peace")

    # ---- A-2 鎮壓回合加碼 ----

    def _riot_turns(self, pre_card):
        engine = GameEngine(seed=5)
        if pre_card:
            self._fire(engine, pre_card)
        self._own(engine, "江蘇", "N")
        payload = engine.state["players"]["F"]
        payload["treasury"] = 500
        payload["hand"].append("du_yuesheng_gamble")
        engine.use_function("F", "du_yuesheng_gamble", target_owner="N", target_province="江蘇")
        return [e["required_turns"] for e in engine.state["city_output_effects"]
                if e.get("kind") == "qing_gang_riot"]

    def test_a_security_crackdown_makes_gang_riots_take_longer_to_quell(self):
        """suppression_turn_bonus() 先前沒有任何 production 呼叫端，只有測試在叫。"""
        self.assertEqual(self._riot_turns(None), [2])
        self.assertEqual(self._riot_turns("burning_red_lotus"), [3])

    def test_the_bonus_reaches_red_army_uprisings_too(self):
        engine = GameEngine(seed=6)
        self._fire(engine, "burning_red_lotus")
        payload = engine.state["players"]["F"]
        payload["treasury"] = 500
        payload["foreign_relations"]["su"] = 8
        payload["hand"].append("red_army_uprising")
        engine.use_function("F", "red_army_uprising", target_owner="N")
        rising = [e for e in engine.state["city_output_effects"]
                  if e.get("kind") == "red_army_uprising"]
        self.assertEqual([e["required_turns"] for e in rising], [3])
        note = engine.state["players"]["N"]["notifications"][-1]["text"]
        self.assertIn("3 回合", note, "通知裡的回合數必須跟實際門檻同一個來源")

    def test_the_bonus_is_locked_in_when_the_riot_starts(self):
        """加碼在發動當下結算一次；治安期過了，已經開著的暴動門檻不變。"""
        engine = GameEngine(seed=5)
        self._fire(engine, "burning_red_lotus")
        self._own(engine, "江蘇", "N")
        payload = engine.state["players"]["F"]
        ...
        payload["hand"].append("du_yuesheng_gamble")
        engine.use_function("F", "du_yuesheng_gamble", target_owner="N", target_province="江蘇")
        for entry in engine.state["suppression_turn_bonuses"]:
            entry["until_turn"] = 0
        self.assertEqual(engine.suppression_turn_bonus(), 0)
        riot = next(e for e in engine.state["city_output_effects"]
                    if e.get("kind") == "qing_gang_riot")
        self.assertEqual(riot["required_turns"], 3)

    # ---- A-3 [幫會]／[學潮] 持續時間加碼 ----

    def _with_and_without(self, source_card, target_card, read, *, tactical=None):
        out = []
        for pre in (None, source_card):
            engine = GameEngine(seed=8)
            if pre:
                self._fire(engine, pre, tactical=tactical)
            self._fire(engine, target_card)
            out.append(read(engine))
        return out

    def test_the_gang_tag_bonus_reaches_city_halt(self):
        plain, boosted = self._with_and_without(
            "opium_and_likin_revenue", "secret_society_trouble",
            lambda e: sorted({x["remaining_turns"] for x in e.state["city_output_effects"]
                              if x.get("kind") == "city_halt"}))
        self.assertEqual(plain, [1])
        self.assertEqual(boosted, [2])

    def test_the_gang_tag_bonus_reaches_action_bans(self):
        plain, boosted = self._with_and_without(
            "opium_and_likin_revenue", "bandit_raids",
            lambda e: [b["until_turn"] - int(e.state["turn"]) for b in e.state["action_bans"]])
        self.assertEqual(plain, [2])
        self.assertEqual(boosted, [3])

    def test_the_gang_tag_bonus_reaches_event_riots(self):
        plain, boosted = self._with_and_without(
            "opium_and_likin_revenue", "gang_unrest",
            lambda e: sorted({x["required_turns"] for x in e.state["city_output_effects"]
                              if x.get("kind") == "qing_gang_riot"}))
        self.assertEqual(plain, [2])
        self.assertEqual(boosted, [3])

    STUDENT_TACTICAL = {"armies": {"C-3": {"generalId": "yang_sen",
                                           "units": {"infantry": 8}, "status": "active"}},
                        "generalOwners": {}, "generalTrees": {}, "jailedGenerals": []}

    def test_the_student_tag_bonus_reaches_city_halt(self):
        plain, boosted = self._with_and_without(
            "wanxian_incident", "student_demonstrations",
            lambda e: sorted({x["remaining_turns"] for x in e.state["city_output_effects"]
                              if x.get("kind") == "city_halt"}),
            tactical=self.STUDENT_TACTICAL)
        self.assertEqual(plain, [3])
        self.assertEqual(boosted, [4])

    def test_the_student_tag_bonus_reaches_student_unrest(self):
        plain, boosted = self._with_and_without(
            "wanxian_incident", "revolutionary_literature",
            lambda e: sorted({x["remaining_turns"] for x in e.state["city_output_effects"]}),
            tactical=self.STUDENT_TACTICAL)
        self.assertEqual(plain, [2])
        self.assertEqual(boosted, [3])

    def test_an_untagged_card_gets_no_bonus(self):
        engine = GameEngine(seed=8)
        self._fire(engine, "opium_and_likin_revenue")
        self.assertEqual(engine._event_duration_bonus(
            engine._event_template("student_demonstrations")), 0,
            "[學潮] 不該吃到 [幫會] 的加碼")

    def test_event_riots_carry_required_turns_not_a_dead_field(self):
        """事件卡開的暴動先前寫的是 suppression_turns，判平息的兩處讀的是
        required_turns——卡片上的門檻因此從來沒被讀過。"""
        engine = GameEngine(seed=8)
        self._fire(engine, "gang_unrest")
        riots = [e for e in engine.state["city_output_effects"]
                 if e.get("kind") == "qing_gang_riot"]
        self.assertTrue(riots)
        for riot in riots:
            self.assertIn("required_turns", riot)
            self.assertNotIn("suppression_turns", riot)

    # ---- A-4 已收買黑金 ----

    def test_no_card_carries_a_prerequisite_nothing_can_grant(self):
        """〈已收買黑金〉這個前置狀態在遊戲裡無從取得，欄位已移除。

        真正的守門在這裡：任何前置條件都必須是遊戲裡拿得到的東西，
        否則卡片要嘛永遠打不出來，要嘛（先前的情況）那條門檻根本沒人讀。
        """
        cards = json.loads(FUNCTION_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        stray = sorted(c["id"] for c in cards if c.get("entry_requirement"))
        self.assertEqual(stray, [], "entry_requirement 沒有任何讀取者，不要再往卡片上加")

    # ---- A-5 eligible_players 要每一家都問到 ----

    RECRUIT_TACTICAL = {"armies": {"C-1": {"generalId": "liu_xiang",
                                           "units": {"infantry": 9}, "status": "active"}},
                        "generalOwners": {}, "generalTrees": {}, "jailedGenerals": []}

    def test_every_eligible_player_gets_to_bid_not_just_the_drawer(self):
        """先前 scope=eligible_players 不算「每家表態」，佇列塌成 [drawer]：
        第一個人回應完卡片就結案，多方競標實戰永遠是 1/1。"""
        engine = GameEngine(seed=4)
        engine.new_game()
        for code in ("F", "N", "S"):
            engine.set_diplomacy(code, "W", "war")
        for payload in engine.state["players"].values():
            payload["treasury"] = 200
        engine.state["event_pool"] = ["liu_xiang_denounces_zhili"]
        current = int(engine.state["turn"])
        engine.state["turn"] = ((current // 3) + 1) * 3 - 1
        engine.next_turn(active_player="F", tactical=dict(self.RECRUIT_TACTICAL))
        view = engine.pending_event_view()
        self.assertEqual(sorted(view["responders"]), ["F", "N", "S"])
        applied = []
        asked = []
        while engine.pending_event_view():
            who = engine.pending_event_view()["waiting_for"]
            asked.append(who)
            applied += engine.respond_event(who, choice="recruit").get("applied") or []
        self.assertEqual(sorted(asked), ["F", "N", "S"], "三家都要被問到")
        entry = next(a for a in applied if a.get("kind") == "contested_npc_recruit")
        self.assertEqual(entry["bidders"], ["F", "N", "S"])
        self.assertEqual(entry["odds"], "1/3")
        self.assertIn(entry["winner"], ("F", "N", "S"))

    def test_an_ineligible_player_is_never_put_in_the_queue(self):
        engine = GameEngine(seed=4)
        engine.new_game()
        for code in ("F", "S"):
            engine.set_diplomacy(code, "W", "war")
        ...
        engine.next_turn(active_player="F", tactical=dict(self.RECRUIT_TACTICAL))
        view = engine.pending_event_view()
        self.assertEqual(sorted(view["responders"]), ["F", "S"], "N 沒對直系宣戰，不該被問")

    def test_every_eligible_players_card_is_treated_as_a_whole_table_vote(self):
        """資料檔上每一張 scope=eligible_players 的卡都要走「每家表態」。"""
        cards = json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        engine = GameEngine(seed=1)
        wanted = [c for c in cards
                  if (c.get("resolution") or {}).get("scope") == "eligible_players"]
        self.assertTrue(wanted, "資料檔應該還有這種卡")
        for card in wanted:
            self.assertTrue(engine.event_needs_every_faction(card), card["id"])

    # ---- A-6 一黨之國要蓋到所有幫會暴動卡 ----

    def test_one_party_state_suspends_every_gang_riot_card(self):
        """新增一張同機制的卡時，封鎖名單漏了它就會靜靜地失效。"""
        cards = json.loads(FUNCTION_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        gang = {c["id"] for c in cards if c.get("mechanic") == "qing_gang_riot"}
        ...
        suspended = set(one_party["apply"]["perk_suspension"]["cards"])
        self.assertEqual(gang - suspended, set(),
                         "〈一黨之國〉沒蓋到的幫會暴動卡")
        for card_id in sorted(gang):
            self.assertIn(f"〈{next(c['name'] for c in cards if c['id'] == card_id)}〉",
                          one_party["effect"], "卡面文字也要列到")


class OrphanedDataTests(unittest.TestCase):
    """資料檔裡「定義了卻沒有任何人用」的東西。

    這一類不會讓遊戲當掉，只會讓卡面／技能表上出現永遠不會發生的字。
    每一條都用明確的白名單守：要留一個孤兒就得寫進白名單，
    而不是讓它靜靜地躺在資料檔裡。
    """

    ENGINE_SOURCE = (REPO_ROOT / "backend" / "card_engine.py").read_text(encoding="utf-8")

    # 引擎有分支、但目前沒有任何卡片宣告的 mechanic。
    # 這三條是刻意留著的骨架（決策：保留，加測試守門）。要新增一條就得先來這裡登記。
    MECHANICS_WITHOUT_A_CARD = {"rail_movement", "rural_movement", "reserve_debt_bundle"}
    # 這兩個不是從卡片的 mechanic 欄位進來的：
    #   loyalty            —— 由 LOYALTY_FUNCTION_CARD_IDS 反推（卡片沒有 mechanic 欄位）
    #   blocked_mechanics  —— 卡片欄位名，不是 mechanic 值
    NOT_A_CARD_MECHANIC = {"loyalty", "blocked_mechanics"}

    def _engine_branch_mechanics(self):
        found = set(re.findall(r'mechanic == "([a-z_0-9]+)"', self.ENGINE_SOURCE))
        for group in re.findall(r'mechanic in \(([^)]*)\)', self.ENGINE_SOURCE):
            found |= set(re.findall(r'"([a-z_0-9]+)"', group))
        return found - self.NOT_A_CARD_MECHANIC

    def _declared_mechanics(self):
        cards = json.loads(FUNCTION_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        return {c["mechanic"] for c in cards if c.get("mechanic")}

    def test_every_card_mechanic_has_an_engine_branch(self):
        missing = sorted(self._declared_mechanics() - self._engine_branch_mechanics())
        self.assertEqual(missing, [], "卡片宣告了引擎接不住的 mechanic")

    def test_engine_branches_without_a_card_are_all_registered(self):
        """空轉的 mechanic 分支必須在白名單上，新增一條就要紅燈。"""
        orphans = self._engine_branch_mechanics() - self._declared_mechanics()
        self.assertEqual(orphans, self.MECHANICS_WITHOUT_A_CARD,
                         "空轉的 mechanic 清單變了——多出來的要嘛補卡、要嘛登記")

    def test_every_defined_trait_is_actually_carried_by_someone(self):
        """特質表裡不該有沒人持有的特質——技能說明會出現在 UI 上卻永遠不會發生。"""
        traits = json.loads(
            (REPO_ROOT / "comabt_system" / "data" / "general_traits.json")
            .read_text(encoding="utf-8"))
        ...
        for path in (REPO_ROOT / "general_tree" / "data").glob("*.json"):
                ...
                if isinstance(node, dict):
                    for key, value in node.items():
                        if key == "traits" and isinstance(value, list):
                            carried.update(t for t in value if isinstance(t, str))
                        walk(value)
                elif isinstance(node, list):
                    for value in node:
                        walk(value)
            ...
            walk(data)
        self.assertEqual(sorted(set(traits) - carried), [],
                         "general_traits.json 裡有沒人持有的特質")

    def test_every_catalogued_skill_is_granted_somewhere(self):
        catalog = json.loads(
            (REPO_ROOT / "general_tree" / "data" / "skill_catalog.json")
            .read_text(encoding="utf-8"))["skills"]
        granted = "\n".join(
            path.read_text(encoding="utf-8")
            for path in (REPO_ROOT / "general_tree" / "data").glob("*.json")
            if path.name != "skill_catalog.json")
        orphans = sorted(name for name in catalog if f'"{name}"' not in granted)
        self.assertEqual(orphans, [], "技能目錄裡有沒人拿得到的技能")

    def test_the_frontend_engineering_trait_map_only_lists_live_traits(self):
        """前端把「哪些特質自帶工事能力」做成一張表；表上掛著沒人持有的特質時，
        那幾行永遠命中不了。"""
        traits = json.loads(
            (REPO_ROOT / "comabt_system" / "data" / "general_traits.json")
            .read_text(encoding="utf-8"))
        traits = set(traits.get("traits", traits))
        block = re.search(r"const ENGINEERING_TRAIT_SKILLS = \{(.*?)\n\};",
                          FRONTEND_SOURCE, re.S)
        self.assertIsNotNone(block)
        listed = set(re.findall(r'"([a-z_0-9]+)"', block.group(1)))
        self.assertEqual(sorted(listed - traits), [],
                         "ENGINEERING_TRAIT_SKILLS 上有已經不存在的特質")


class SingleSourceOfTruthTests(unittest.TestCase):
    """同一條規則不准存在兩份。這一組守的是幾個「卡片寫了、程式又自備一份」的欄位。"""

    def _engine(self):
        engine = GameEngine(seed=3)
        payload = engine.state["players"]["F"]
        payload["treasury"] = 500
        payload["unlocks"].append("event_penicillin")
        payload["hand"].append("penicillin_import")
        engine.use_function("F", "penicillin_import",
                            target_general_id="wu_peifu", target_owner="F")
        engine.state["turn"] = 5
        return engine

    @staticmethod
    def _tactical():
        return {"armies": {"F-1": {"generalId": "wu_peifu", "faction": "F", "status": "active",
                                   "units": {"infantry": 5},
                                   "fieldHospitalPending": {"turn": 4,
                                                            "units": ["infantry"]}}}}

    def test_the_field_hospital_reads_recover_battalions_from_the_card(self):
        """先前引擎寫死 +1，卡片上的 recover_battalions 沒有任何讀取者。"""
        for battalions in (1, 2, 3):
            engine = self._engine()
            for card in engine.data["function_cards"]["cards"]:
                if card.get("mechanic") == "field_hospital":
                    card["recover_battalions"] = battalions
            healed = engine.field_hospital_recovery(self._tactical(), 5)["healed"]
            self.assertEqual(healed[0]["units"]["infantry"], 5 + battalions, battalions)

    def test_two_field_hospital_cards_with_different_numbers_raise(self):
        """真要出現第二張數字不同的卡，就該當場紅燈，不要偷偷挑第一張。"""
        engine = self._engine()
        extra = deepcopy(next(c for c in engine.data["function_cards"]["cards"]
                              if c.get("mechanic") == "field_hospital"))
        extra["id"] = "another_field_hospital"
        extra["recover_battalions"] = 2
        engine.data["function_cards"]["cards"].append(extra)
        with self.assertRaisesRegex(ValueError, "recover_battalions"):
            engine.field_hospital_recovery(self._tactical(), 5)

    def test_general_bound_perks_come_from_the_cards_lost_on_defection_flag(self):
        engine = GameEngine(seed=3)
        self.assertEqual(sorted(engine.GENERAL_BOUND_PERK_KEYS),
                         ["field_hospital_generals", "permanent_forced_march_generals"])
        for card in engine.data["function_cards"]["cards"]:
            if card["id"] == "penicillin_import":
                card.pop("lost_on_defection")
        self.assertEqual(engine.GENERAL_BOUND_PERK_KEYS,
                         ("permanent_forced_march_generals",),
                         "名單必須跟著卡片欄位走，不是寫死的 tuple")

    def test_an_unmapped_lost_on_defection_card_raises_instead_of_being_ignored(self):
        engine = GameEngine(seed=3)
        engine.data["function_cards"]["cards"].append(
            {"id": "made_up", "mechanic": "no_such_mechanic", "lost_on_defection": True})
        with self.assertRaisesRegex(ValueError, "PERK_ROSTER_BY_MECHANIC"):
            engine.drop_general_bound_perks("wu_peifu")

    def test_the_defection_drop_still_works_end_to_end(self):
        engine = self._engine()
        payload = engine.state["players"]["F"]
        self.assertEqual(payload["field_hospital_generals"], ["wu_peifu"])
        engine.apply_general_join("W", [], general_id="wu_peifu")
        self.assertEqual(payload["field_hospital_generals"], [])

    def test_clear_cards_and_the_academia_purge_share_one_implementation(self):
        """〈中央研究院〉先前自帶一份清除迴圈，通用的 clear_cards 因此沒人走得到。"""
        engine = GameEngine(seed=3)
        payload = engine.state["players"]["F"]
        payload["hand"].append("national_shame")
        payload["discard"].append("national_shame")
        payload["function_deck"].append("national_shame")
        removed = engine._purge_card_everywhere("F", "national_shame")
        self.assertEqual(removed, 3)
        for zone in ("hand", "discard", "function_deck"):
            self.assertNotIn("national_shame", payload[zone])

    def test_the_generic_clear_cards_payload_actually_clears(self):
        engine = GameEngine(seed=3)
        payload = engine.state["players"]["F"]
        payload["hand"].append("national_shame")
        applied = engine._apply_event_payload({"clear_cards": ["national_shame"]},
                                              players=["F"],
                                              card={"id": "probe", "name": "probe"})
        self.assertEqual([a["kind"] for a in applied], ["clear_cards"])
        self.assertNotIn("national_shame", payload["hand"])

    DEAD_CARD_FIELDS = ("entry_requirement", "anti_communist", "base_weight", "conditions")

    def test_removed_card_fields_do_not_come_back(self):
        """這幾個欄位都沒有讀取者，值改了不會有任何效果——不要再往卡片上寫。"""
        cards = json.loads(FUNCTION_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        for field in self.DEAD_CARD_FIELDS:
            owners = sorted(c["id"] for c in cards if field in c)
            self.assertEqual(owners, [], f"{field} 沒有任何讀取者")

    PRODUCTION_SOURCES = ("backend/card_engine.py", "backend/server.py",
                          "backend/combat_modifiers.py", "backend/foreign_punishment.py",
                          "frontend/app.js")

    def test_these_helpers_have_a_real_caller_not_just_tests(self):
        """suppression_turn_bonus() 曾經是「只有測試在叫」的公開方法。"""
        source = "\n".join((REPO_ROOT / rel).read_text(encoding="utf-8")
                            for rel in self.PRODUCTION_SOURCES)
        for name in ("suppression_turn_bonus()", "_extended_duration(",
...
class BackendIsTheOnlyEngineTests(unittest.TestCase):
    """後端是唯一的計算引擎，前端只呈現。

    這一組守的是「前端不准自己有一份」。做法一律是**全檔搜字串**：只要那個公式、
    那張表、那個裸數字回到前端，就紅。比對兩邊算出同一個值的做法已經廢掉——
    那不是單一來源，那是兩份規則加一個看門的。
    """

    # 前端不准再出現的東西：規則公式的片段、規則表名、以及規則數字的字面值。
    FORBIDDEN = {
        "策反公式": ("0.45 - ", "* 1.25", "Math.min(0.60"),
        "抗策反表": ("DEFECTION_RESISTANCE_TRAITS", "defectionResistance"),
        "忠誠算式": ("relativePower", "battleLoss", "traitLoyaltyAdjustment"),
        "技能失效判準表": ("RELATION_DISABLED_TRAITS = {",),
        "規則數字的第二份": ("MAX_HAND_SIZE = 6", "DEFAULT_FUNCTION_CARD_DRAW_COST",
                             "ENGINEERING_FALLBACK", "const FORCED_MARCH = {",
                             "army_force_cap || 100", "tiles_per_turn || 2",
                             "factory_cost_per_gun_boat || 5", "gun_boat?.hp || 30"),
    }

    def test_the_frontend_keeps_no_copy_of_any_backend_rule(self):
        for label, fragments in self.FORBIDDEN.items():
            for fragment in fragments:
                self.assertNotIn(fragment, FRONTEND_SOURCE,
                                 f"{label}：前端又出現「{fragment}」")

    def test_rule_numbers_come_from_the_backend_or_show_a_dash(self):
        """拿不到後端的數字時要顯示「—」，不是端出一個看起來很像真的數字。"""
        self.assertIn("function backendRule(", FRONTEND_SOURCE)
        self.assertIn("function showNumber(", FRONTEND_SOURCE)
        for getter in ("function functionCardDrawCost", "function functionCardMaxHandSize",
                       "function armyForceCap", "function forcedMarchRules",
                       "function engineeringRules"):
            body = FRONTEND_SOURCE[FRONTEND_SOURCE.index(getter):]
            body = body[:body.index("\n}") + 2]
            self.assertNotRegex(body, r"\|\|\s*\d", f"{getter} 還留著寫死的退路值")

    def test_every_rule_number_the_ui_shows_is_exported_by_the_backend(self):
        """畫面要用的規則數字，後端都得送出來——不然前端只能自己編。"""
        engine = GameEngine(seed=1)
        boot = engine.bootstrap()
        for path in (("features", "function_card_draw_cost"),
                     ("features", "function_card_draw_factory_cost"),
                     ("features", "function_card_purchase_limit"),
                     ("features", "function_card_max_hand_size"),
                     ("features", "army_force_cap"),
                     ("features", "unit_force_points"),
                     ("features", "forced_march"),
                     ("engineering", "pontoon_bridge"),
                     ("navy_system", "move"),
                     ("navy_system", "repair")):
            node = boot
            for key in path:
                self.assertIn(key, node, ".".join(path))
                node = node[key]

    def test_the_snapshot_carries_every_derived_value_the_ui_renders(self):
        """忠誠、失效技能、已解算價目——畫面要顯示的都得在後端算好隨狀態送出。"""
        engine = GameEngine(seed=1)
        snapshot = engine.snapshot()
        for player, payload in snapshot["players"].items():
            for key in ("disabled_traits", "resolved_recruit_costs", "resolved_navy_costs"):
                self.assertIn(key, payload, f"{player} 少了 {key}")
        self.assertIn("quellable_unrest", snapshot)
        server_source = (REPO_ROOT / "backend" / "server.py").read_text(encoding="utf-8")
        for key in ('"loyalty": ENGINE.loyalty_report',
                    '"navy_outlook": ENGINE.navy_outlook',
                    '"railway_access": ENGINE.railway_access'):
            self.assertIn(key, server_source, f"shared-state 沒送 {key}")

    def test_disabled_traits_follow_the_backend_table(self):
        engine = GameEngine(seed=3)
        payload = engine.state["players"]["F"]
        payload["foreign_relations"]["su"] = -10
        self.assertEqual(engine.disabled_traits("F"), [])
        payload["foreign_relations"]["su"] = 9
        self.assertEqual(engine.disabled_traits("F"),
                         ["anticommunist_vanguard", "white_russian_mercenaries"])
        from backend.combat_modifiers import RELATION_DISABLED_TRAITS
        self.assertEqual(set(engine.disabled_traits("F")), set(RELATION_DISABLED_TRAITS))

    def test_the_defection_quote_reads_the_loyalty_from_the_snapshot(self):
        """突變測試逃掉過一次：報價把 loyalty_report 換成 {} 也沒人紅。

        忠誠越高越難策反、也越貴——所以同一支部隊換一個忠誠，報價就得跟著動。
        """
        engine = GameEngine(seed=3)
        ...
        def quote(loyalty):
            tactical = {
                "armies": {"W-1": {"generalId": "wu_peifu", "faction": "W", "status": "active",
                                   "units": {"infantry": 10}}},
                "generalTrees": {"W": {"generals": {
                    "wu_peifu": {"id": "wu_peifu", "loyalty": loyalty, "traits": []}}}},
                "generalOwners": {"wu_peifu": "W"}, "loyaltyOverrides": {},
            }
            return engine.defection_quote("wu_peifu", tactical)

        low, high = quote(2), quote(9)
        self.assertEqual(low["loyalty"], engine.loyalty_report(
            {"armies": {"W-1": {"generalId": "wu_peifu", "faction": "W", "status": "active",
                                "units": {"infantry": 10}}},
             "generalTrees": {"W": {"generals": {
                 "wu_peifu": {"id": "wu_peifu", "loyalty": 2, "traits": []}}}},
             "generalOwners": {"wu_peifu": "W"}, "loyaltyOverrides": {}})["wu_peifu"]["value"],
            "報價的忠誠必須就是 loyalty_report 算出來的那一個")
        self.assertNotEqual(low["loyalty"], high["loyalty"], "忠誠沒有從快照讀進來")
        self.assertLess(high["chance"], low["chance"], "忠誠高就該更難策反")
        self.assertGreater(high["cost"], low["cost"], "忠誠高就該更貴")

    def test_the_snapshot_disabled_traits_are_real_not_an_empty_placeholder(self):
        """突變測試逃掉過一次：snapshot 裡塞死 [] 也沒人紅。"""
        engine = GameEngine(seed=3)
        engine.state["players"]["F"]["foreign_relations"]["su"] = 9
        payload = engine.snapshot()["players"]["F"]
        self.assertEqual(payload["disabled_traits"], engine.disabled_traits("F"))
        self.assertNotEqual(payload["disabled_traits"], [], "snapshot 沒把真的名單送出去")
        engine.state["players"]["F"]["foreign_relations"]["su"] = -10
        self.assertEqual(engine.snapshot()["players"]["F"]["disabled_traits"], [])

    def test_the_rule_getters_return_the_backend_value_and_nothing_else(self):
        """突變測試逃掉過一次：`return 6;` 這種寫死法躲得過「|| 數字」的檢查。

        改成逐個函式檢查它的函式體裡除了 backendRule／backendNumber 之外
        不准出現任何裸數字。
        """
        getters = ("functionCardDrawCost", "functionCardDrawFactoryCost",
                   "functionCardMaxHandSize", "armyForceCap")
        for name in getters:
            body = FRONTEND_SOURCE[FRONTEND_SOURCE.index(f"function {name}("):]
            body = body[body.index("{") + 1:]
            body = body[:body.index("\n}")]
            self.assertRegex(body, r"backend(Rule|Number)\(",
                             f"{name} 沒有跟後端要值")
            self.assertNotRegex(body, r"(?<![\w.\[])\d+(?![\w\]])",
                                f"{name} 的函式體裡出現了寫死的數字")

    def test_the_ui_probe_harness_exists_and_can_move_every_number(self):
        """隨機調後端數字看畫面跟不跟得動的那支腳本，本身也要在。"""
        harness = REPO_ROOT / "scripts" / "checks" / "backend_drives_ui_e2e.py"
        launcher = REPO_ROOT / "scripts" / "checks" / "_probe_server.py"
        self.assertTrue(harness.exists() and launcher.exists())
        text = launcher.read_text(encoding="utf-8")
        for knob in ("function_card_max_hand_size", "function_card_draw_cost",
                     "army_force_cap", "unit_force_points", "forced_march",
                     "ENGINEERING_OPERATIONS", "DEFECTION_RESISTANCE_TRAITS",
                     "factory_cost_per_gun_boat"):
            self.assertIn(knob, text, f"探針沒有動到 {knob}")
        # 產品程式碼裡不准有測試旗標——探針只在啟動器裡改值。
        for rel in ("backend/card_engine.py", "backend/server.py", "frontend/app.js"):
            self.assertNotIn("NE_UI_PROBE", (REPO_ROOT / rel).read_text(encoding="utf-8"), rel)


class LoyaltyDeltasActuallyLandTests(unittest.TestCase):
    """功能卡與事件卡的忠誠加減。

    先前這一段在前端，而且加在**畫面顯示值**上——顯示值已經含了相對實力與戰損的
    修正，後端拿它當基礎再套一次，於是「忠誠 +2」在強軍身上縮成 +1、
    在弱軍身上直接變成 **+0**，卡等於完全沒效果。加減現在只在後端做，加的是基礎值。
    """

    def _tactical(self, base=7):
        return {
            "armies": {
                # 一支很大、一支很小——相對實力的修正一正一負，
                # 正好照出「加在顯示值上」與「加在基礎值上」的差別。
                "W-1": {"generalId": "wu_peifu", "faction": "W", "status": "active",
                        "units": {"infantry": 30, "cavalry": 5, "machine_gun": 2, "artillery": 2}},
                "W-2": {"generalId": "jin_yun_e", "faction": "W", "status": "active",
                        "units": {"infantry": 6}},
            },
            "generalTrees": {"W": {"generals": {
                "wu_peifu": {"id": "wu_peifu", "loyalty": base, "traits": []},
                "jin_yun_e": {"id": "jin_yun_e", "loyalty": base, "traits": []}}}},
            "generalOwners": {"wu_peifu": "W", "jin_yun_e": "W"},
            "loyaltyOverrides": {}, "loyaltyBaselineArmyUnits": {},
        }

    def test_a_plus_two_really_raises_the_base_by_two(self):
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        overrides = engine.apply_loyalty_deltas([{"owner": "W", "amount": 2}], tactical)
        self.assertEqual(overrides, {"wu_peifu": 9, "jin_yun_e": 9},
                         "加的必須是基礎值 7 → 9")

    def test_the_weak_army_case_that_used_to_come_out_as_zero(self):
        """靳雲鶚（小部隊、相對實力 −2）先前吃到的實際變化是 +0。"""
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        before = engine.loyalty_report(tactical)["jin_yun_e"]["value"]
        overrides = engine.apply_loyalty_deltas([{"owner": "W", "amount": 2}], tactical)
        after = engine.loyalty_report({**tactical, "loyaltyOverrides": overrides})["jin_yun_e"]["value"]
        self.assertEqual(after - before, 2, f"忠誠 +2 實際只變了 {after - before}")

    def test_a_minus_one_really_lowers_it(self):
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        before = engine.loyalty_report(tactical)["jin_yun_e"]["value"]
        overrides = engine.apply_loyalty_deltas([{"owner": "W", "amount": -1}], tactical)
        after = engine.loyalty_report({**tactical, "loyaltyOverrides": overrides})["jin_yun_e"]["value"]
        self.assertEqual(after - before, -1)

    def test_deltas_stack_on_the_previous_override_not_on_the_base(self):
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        first = engine.apply_loyalty_deltas([{"owner": "W", "amount": 2}], tactical)
        second = engine.apply_loyalty_deltas(
            [{"owner": "W", "amount": 2}], {**tactical, "loyaltyOverrides": first})
        self.assertEqual(second["jin_yun_e"], 10, "第二次要從 9 再加，不是從 7 重來")

    def test_the_override_is_clamped_between_one_and_ten(self):
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        self.assertEqual(engine.apply_loyalty_deltas(
            [{"owner": "W", "amount": 99}], tactical)["wu_peifu"], 10)
        self.assertEqual(engine.apply_loyalty_deltas(
            [{"owner": "W", "amount": -99}], tactical)["wu_peifu"], 1)

    def test_absolute_and_exempt_generals_are_left_alone(self):
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        tactical["generalTrees"]["W"]["generals"]["wu_peifu"]["absolute_loyalty"] = True
        tactical["generalTrees"]["W"]["generals"]["jin_yun_e"]["loyalty_exempt"] = True
        self.assertEqual(engine.mutable_loyalty_generals("W", tactical), [])
        self.assertEqual(engine.apply_loyalty_deltas([{"owner": "W", "amount": 2}], tactical), {})

    def test_using_a_loyalty_card_returns_the_new_overrides(self):
        """功能卡打出去之後，回應裡要帶著新的 loyaltyOverrides 讓前端照抄。

        〈國家的干城〉限國民革命軍，所以把那兩位將領放在 N 底下再打。
        """
        engine = GameEngine(seed=3)
        card = next(c for c in engine.data["function_cards"]["cards"]
                    if c.get("mechanic") == "loyalty_all"
                    and int(c.get("loyalty_delta", 0)) > 0
                    and engine._card_allowed_for_player(c["id"], "N"))
        tactical = self._tactical()
        tactical["generalTrees"] = {"N": tactical["generalTrees"]["W"]}
        tactical["generalOwners"] = {"wu_peifu": "N", "jin_yun_e": "N"}
        for army in tactical["armies"].values():
            army["faction"] = "N"
        engine._tactical = tactical
        payload = engine.state["players"]["N"]
        payload["treasury"] = 500
        payload["hand"].append(card["id"])
        result = engine.use_function("N", card["id"])
        self.assertTrue(result["loyalty_overrides"], "回應裡沒有新的 overrides")
        amount = int(card["loyalty_delta"])
        for general_id, value in result["loyalty_overrides"].items():
            self.assertEqual(value, min(10, 7 + amount), general_id)

    def test_the_event_channel_resolves_targets_and_overrides_in_the_backend(self):
        engine = GameEngine(seed=3)
        tactical = self._tactical()
        everyone = engine.resolve_loyalty_effect(
            "loyalty_all", "W", {"amount": 1}, tactical)
        self.assertEqual(sorted(everyone["picked"]), ["jin_yun_e", "wu_peifu"])
        self.assertEqual(everyone["overrides"], {"wu_peifu": 8, "jin_yun_e": 8})
        one = engine.resolve_loyalty_effect(
            "loyalty_random", "W", {"amount": -1, "count": 1}, tactical)
        self.assertEqual(len(one["picked"]), 1, "抽籤要在後端做，而且照 count 抽")
        self.assertEqual(list(one["overrides"]), one["picked"])

    def test_the_random_pick_is_reproducible_from_the_seed(self):
        """抽籤搬到後端的意義之一：同一顆種子重播結果一致。"""
        picks = set()
        for _ in range(5):
            engine = GameEngine(seed=11)
            picks.add(tuple(engine.resolve_loyalty_effect(
                "loyalty_random", "W", {"amount": -1, "count": 1}, self._tactical())["picked"]))
        self.assertEqual(len(picks), 1, f"同一顆種子抽出不同結果：{picks}")

    def test_the_frontend_only_copies_what_the_backend_sends(self):
        self.assertNotIn("function adjustGeneralLoyalty", FRONTEND_SOURCE,
                         "前端又自己算忠誠加減了")
        self.assertIn("function applyLoyaltyOverrides", FRONTEND_SOURCE)
        for handler in ("loyalty_all", "loyalty_random"):
            block = FRONTEND_SOURCE[FRONTEND_SOURCE.index(f"  {handler}: async ("):]
            block = block[:block.index("\n  },")]
            self.assertIn('/api/loyalty-effect', block, f"{handler} 沒跟後端要結果")
            self.assertNotIn("Math.random", block, f"{handler} 還在前端抽籤")


class FunctionCardSummaryTests(unittest.TestCase):
    """出牌摘要不能把生效的卡說成「無效果，浪費一次出牌」。"""

    def _summary_body(self):
        body = FRONTEND_SOURCE[:FRONTEND_SOURCE.index("：已生效（本卡的效果沒有可顯示的摘要）。")]
        return body[body.rindex("function "):]

    def test_every_result_field_the_backend_returns_has_a_description(self):
        engine_source = (REPO_ROOT / "backend" / "card_engine.py").read_text(encoding="utf-8")
        start = engine_source.index('            "card": card,\n            "target_general_id"')
        block = engine_source[start:engine_source.index('"state": self.snapshot(),', start)]
        # loyalty_overrides 不是一種效果，是「忠誠加減算完之後的新基礎值」，
        # 給前端照抄用；摘要描述的是 loyalty_delta / _all / _swings 那幾個。
        fields = set(re.findall(r'"([a-z_]+)":', block)) - {"card", "state", "loyalty_overrides"}
        body = self._summary_body()
        missing = sorted(f for f in fields if f"action.{f}" not in body)
        self.assertEqual(missing, [],
                         f"這些效果生效了卻沒有摘要，畫面會說「無效果」：{missing}")

    def test_the_fallback_no_longer_claims_the_card_was_wasted(self):
        self.assertNotIn("無效果，浪費一次出牌", FRONTEND_SOURCE,
                         "摘要寫不出來不等於卡沒效果——後端已經扣過錢了")

    def test_each_undescribed_field_has_its_own_guard_branch(self):
        """突變測試逃掉過一次：把 `if (action.railway_effect) {` 改成 `if (false) {`，
        而字串 `action.railway_effect` 還在區塊裡，只檢查字串的測試就抓不到。
        所以改成檢查**判斷式本身**。
        """
        body = self._summary_body()
        for field in ("railway_effect", "port_demolition", "cabinet_entry", "exile_recruit"):
            self.assertIn(f"if (action.{field}) {{", body,
                          f"{field} 的摘要分支被拆掉了")
        self.assertIn("for (const change of action.relation_side_effects || [])", body)

    def test_the_railway_sabotage_result_carries_what_the_summary_needs(self):
        """崩鐵玩家：先前這張卡打出去，摘要一律說「無效果，浪費一次出牌」。"""
        engine = GameEngine(seed=3)
        payload = engine.state["players"]["F"]
        payload["treasury"] = 500
        payload["factory_points"] = 500
        card = next(c for c in engine.data["function_cards"]["cards"]
                    if c.get("mechanic") == "railway_sabotage")
        payload["hand"].append(card["id"])
        railway = (card.get("railways") or [
            line["name"] for line in engine.data["strategic_map"]["railroads"]])[0]
        result = engine.use_function("F", card["id"], target_railway=railway)
        effect = result["railway_effect"]
        self.assertTrue(effect)
        for field in ("railway", "remaining_turns", "repair_factory_cost"):
            self.assertIn(field, effect, field)
        self.assertIn("action.railway_effect", self._summary_body())


class CityDisruptionTagsTests(unittest.TestCase):
    """被暴動／停產壓著的城市，地格資訊欄要看得到是哪一種、還差多少。"""
    ...
    def _engine(self):
        ...
        payload["treasury"] = 500
        payload["foreign_relations"]["su"] = 8
        payload["hand"] += ["du_yuesheng_gamble", "red_army_uprising", "communist_riot"]
        engine.use_function("F", "du_yuesheng_gamble", target_owner="N", target_province="江蘇")
        engine.use_function("F", "red_army_uprising", target_owner="W")
        engine.use_function("F", "communist_riot", target_owner="S")
        return engine

    def test_every_disrupted_city_is_in_the_report(self):
        engine = self._engine()
        report = engine.city_disruption_report()
        listed = {city_id for city_id in report}
        affected = {city_id for effect in engine.state["city_output_effects"]
                    for city_id in (effect.get("city_ids") or [])}
        self.assertEqual(listed, affected, "有城市被壓著卻不在報告裡")
        self.assertTrue(affected)

    def test_the_three_riot_kinds_report_the_right_shape(self):
        engine = self._engine()
        report = engine.city_disruption_report()
        seen = {}
        ...
        self.assertEqual(sorted(seen), ["communist_riot", "qing_gang_riot", "red_army_uprising"])
        self.assertEqual(seen["qing_gang_riot"]["label"], "黑幫暴動")
        self.assertEqual(seen["red_army_uprising"]["label"], "紅軍起義")
        self.assertEqual(seen["communist_riot"]["label"], "共黨暴動")
        for kind in ("qing_gang_riot", "red_army_uprising"):
            self.assertEqual(seen[kind]["mode"], "garrison")
            self.assertIn("progress", seen[kind])
            self.assertGreaterEqual(seen[kind]["required_turns"], 2)
        self.assertEqual(seen["communist_riot"]["mode"], "timed")
        self.assertEqual(seen["communist_riot"]["elapsed_turns"], 0)
        self.assertEqual(seen["communist_riot"]["total_turns"],
                         seen["communist_riot"]["remaining_turns"])

    def test_the_garrison_progress_moves_with_the_backend(self):
        engine = self._engine()
        riot = next(e for e in engine.state["city_output_effects"]
                    if e.get("kind") == "qing_gang_riot")
        city_id = riot["city_ids"][0]
        self.assertEqual(engine.city_disruption_report()[city_id][0]["progress"], 0)
        engine.next_turn(active_player="F", riot_garrisons={riot["id"]: True}, force=True)
        after = engine.city_disruption_report().get(city_id)
        self.assertTrue(after, "駐紮一回合之後這座城還在暴動中")
        self.assertEqual(after[0]["progress"], 1, "進度沒有跟著駐軍走")

    def test_the_red_army_progress_is_per_city(self):
        """紅軍起義的鎮壓進度是**逐城**記的（黑幫暴動是整省一個計數）。

        突變測試逃掉過一次：把逐城取值改成一律 0 也沒人紅。
        """
        engine = self._engine()
        rising = next(e for e in engine.state["city_output_effects"]
                      if e.get("kind") == "red_army_uprising")
        first, second = rising["city_ids"][0], rising["city_ids"][1]
        rising["garrison_progress"] = {first: 1, second: 0}
        report = engine.city_disruption_report()
        self.assertEqual(report[first][0]["progress"], 1, "駐滿一回合的那座城進度沒算到")
        self.assertEqual(report[second][0]["progress"], 0)

    def test_the_report_rides_along_in_the_snapshot(self):
        engine = self._engine()
        self.assertEqual(engine.snapshot()["city_disruptions"], engine.city_disruption_report())

    def test_the_frontend_reads_the_report_instead_of_scanning_effects(self):
        self.assertIn("state?.city_disruptions?.[cityId]", FRONTEND_SOURCE)
        block = FRONTEND_SOURCE[FRONTEND_SOURCE.index("function cityDisruptions("):]
        block = block[:block.index("\n}")]
        self.assertNotIn("city_output_effects", block, "前端又自己掃 city_output_effects 了")
        self.assertIn("tile-tag-disruption", FRONTEND_SOURCE, "地格資訊欄沒有癱瘓標籤")


class IntelAndPoliceCardsTests(unittest.TestCase):
    """情報網 / 情報局 / 警政單位 / 黑幫暴動：四者互不相干，卡面也要說對。"""

    def _cards(self):
        return {c["id"]: c for c in
                json.loads(FUNCTION_CARDS_PATH.read_text(encoding="utf-8"))["cards"]}

    def test_the_intel_network_card_names_the_right_blocker(self):
        """擋情報網的是情報局，不是警政單位。卡面先前寫錯了對象。"""
        text = self._cards()["intel_network"]["effect"]
        self.assertIn("情報局", text)
        self.assertNotIn("警政", text)

    def test_the_two_cards_no_longer_share_a_confusable_mechanic_name(self):
        cards = self._cards()
        self.assertEqual(cards["police_system"]["mechanic"], "counter_intel")
        self.assertEqual(cards["police_precinct"]["mechanic"], "gang_riot_shield")

    def test_the_counter_intel_flag_blocks_only_the_intel_network(self):
        engine = GameEngine(seed=3)
        payload = engine.state["players"]["W"]
        payload["treasury"] = 500
        payload["hand"].append("police_system")
        engine.use_function("W", "police_system")
        observer = engine.state["players"]["F"]
        observer["timed_effects"].append(
            {"kind": "intel_network", "target_province": "直隸", "remaining_turns": 1})
        report = engine.intel_report("F")
        self.assertIn("直隸", report["intel_provinces"])
        self.assertIn("W", report["counter_intel_factions"])
        # 飛艇無視反情報
        observer["timed_effects"].append(
            {"kind": "aerial_recon", "target_provinces": ["山東"], "remaining_turns": 1})
        self.assertIn("山東", engine.intel_report("F")["aerial_provinces"])

    def test_aerial_recon_sees_through_counter_intel(self):
        """飛艇在雲上照相，情報局擋不住——這條先前沒有任何測試在守。"""
        engine = GameEngine(seed=3)
        engine.state["players"]["W"]["timed_effects"].append(
            {"kind": "counter_intel", "remaining_turns": 3, "owners": ["W"]})
        observer = engine.state["players"]["F"]
        observer["timed_effects"].append(
            {"kind": "intel_network", "target_province": "直隸", "remaining_turns": 1})
        observer["timed_effects"].append(
            {"kind": "aerial_recon", "target_provinces": ["山東"], "remaining_turns": 1})
        report = engine.intel_report("F")
        self.assertIn("W", report["counter_intel_factions"], "情報局要擋得住情報網")
        self.assertIn("山東", report["aerial_provinces"], "飛艇照到的省要照樣回報")
        # 前端的判定順序：飛艇 > 情報局 > 情報網。這裡守的是那個順序沒有被拿掉。
        block = FRONTEND_SOURCE[FRONTEND_SOURCE.index("function provinceRevealedTo("):]
        block = block[:block.index("\n}")]
        self.assertLess(block.index("aerial_provinces"), block.index("counter_intel_factions"),
                        "飛艇必須排在情報局之前判，否則情報局會把飛艇也擋掉")
        self.assertIn("intel.aerial_provinces.includes(province)) return true", block)

    def test_the_police_precinct_shield_blocks_only_gang_riots(self):
        """警政單位擋的是黑幫暴動，不是共黨暴動、紅軍起義，更不是情報網。"""
        engine = GameEngine(seed=3)
        for city in engine.data["strategic_map"]["cities"]:
            if city.get("province") == "河南":
                engine.state["city_owners"][city["id"]] = "F"
        payload = engine.state["players"]["F"]
        payload["treasury"] = 500
        payload["hand"].append("police_precinct")
        engine.use_function("F", "police_precinct", target_province="河南")
        self.assertTrue(engine._gang_riot_shielded("F", "河南", "qing_gang_riot"))
        for mechanic in ("communist_riot", "red_army_uprising", "security_event"):
            self.assertFalse(engine._gang_riot_shielded("F", "河南", mechanic), mechanic)
        self.assertEqual(engine.intel_report("F")["counter_intel_factions"], [],
                         "警政單位不該擋情報網")

    def test_the_gang_event_cards_respect_the_precinct_and_nothing_else_does(self):
        cards = json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        respecting = sorted(c["name"] for c in cards
                            if "respect_police" in json.dumps(c, ensure_ascii=False))
        self.assertEqual(respecting, ["會黨滋事", "黑幫動亂"])
        precinct = self._cards()["police_precinct"]["effect"]
        for name in respecting:
            self.assertIn(name, precinct, f"警政單位擋得到〈{name}〉，卡面卻沒寫")

    def test_the_visibility_rule_lives_in_the_backend(self):
        block = FRONTEND_SOURCE[FRONTEND_SOURCE.index("function provinceRevealedTo("):]
        block = block[:block.index("\n}")]
        self.assertIn("state?.intel?.[observer]", block)
        for fragment in ('activeTimedEffects(observer, "aerial_recon")',
...
class BlockingAndNpcTransferTests(unittest.TestCase):
    """鐵路阻截、急行軍支援、NPC 轉屬重編番號、吞併類地盤易主。

    行為本身由 scripts/checks/blocking_and_npc_transfer_e2e.py 在真前端上驗；
    這裡守的是「規則沒有被搬回兩份」與「沒有再冒出沒人讀的狀態字串」。
    """

    def _block(self, name: str) -> str:
        start = FRONTEND_SOURCE.index(f"function {name}(")
        return FRONTEND_SOURCE[start:FRONTEND_SOURCE.index("\n}", start)]

    def test_rail_and_forced_march_share_one_blocking_rule(self):
        # 阻截判定只能有一份。兩條路徑都得走 transitBlockedAt，
        # 不能各自再抄一段「這格有沒有敵軍」。
        for name in ("railwayPath", "forcedMarchPath"):
            self.assertIn("transitBlockedAt(", self._block(name),
                          f"{name} 沒有走共用的阻截判定")
        self.assertEqual(FRONTEND_SOURCE.count("function transitBlockedAt("), 1)
        self.assertEqual(FRONTEND_SOURCE.count("function blockingArmyAtCell("), 1)

    def test_the_battle_support_range_is_the_movement_range(self):
        # 支援範圍不能自己再寫一套格數，要問 forcedMarchPath。
        block = self._block("armyCanReachBattleCell")
        self.assertIn("forcedMarchPath(", block)
        self.assertNotIn("cellWithinRange", block)
        self.assertIn("armyCanReachBattleCell(", self._block("joinableBattleForArmy"))

    def test_npc_transfers_go_through_one_renumbering_path(self):
        # 三個 NPC 交辦處理器都得走同一個重編函式，不能各自寫一份 army.faction = ...
        block = FRONTEND_SOURCE[FRONTEND_SOURCE.index("npc_general_transferred:"):
                                FRONTEND_SOURCE.index("loyalty_all: async")]
        for handler in ("npc_general_transferred", "npc_general_recruited",
                        "npc_faction_absorbed"):
            section = block[block.index(f"{handler}:"):]
            section = section[:section.index("\n  },")]
            self.assertIn("reassignGeneralAndArmy(", section,
                          f"{handler} 沒有走重編番號／換將領樹的共用路徑")
        self.assertIn("formatArmyDesignator(nextAvailableArmyNumber(",
                      self._block("reassignGeneralAndArmy"))

    def test_annexed_cities_are_painted_on_the_map(self):
        # 後端把城市記在 city_owners，地圖畫的是 city.faction 與 cell.fac。
        # 吞併類的兩個處理器都要把後端算好的名單畫上去。
        block = FRONTEND_SOURCE[FRONTEND_SOURCE.index("npc_faction_absorbed:"):
                                FRONTEND_SOURCE.index("loyalty_all: async")]
        self.assertEqual(block.count("applyBackendCityTransfers(effect.cities"), 2)
        painter = self._block("applyBackendCityTransfers")
        self.assertIn("cell.fac = faction", painter)
        self.assertIn("transferCityEconomy(", painter)
        # 這條路不該再打一次佔領 API——城已經是後端自己轉的。
        self.assertNotIn("/api/capture-city", painter)

    def test_no_army_status_that_nothing_recognises(self):
        # 「寫了但沒人讀」的狀態字串是這個專案反覆出事的地方：
        # 被併掉的部隊先前掛 "merged"，前後端都沒有一處認得。
        known = set(GameEngine.DEAD_ARMY_STATUSES) | {"active"}
        engine_source = (REPO_ROOT / "backend" / "card_engine.py").read_text(encoding="utf-8")
        written = set(re.findall(r'army\["status"\]\s*=\s*"([a-z_]+)"', engine_source))
        self.assertTrue(written, "抓不到後端寫部隊狀態的地方，這條守門等於沒跑")
        self.assertFalse(written - known,
                         f"後端寫了沒人認得的部隊狀態：{written - known}")
        # 前端把「已經不在場上」的名單抄了好幾份；後端寫的每個退場狀態
        # 都必須被每一份抄本認得，否則空殼部隊會留在地圖上。
        filters = re.findall(r'\[("(?:jailed|killed|destroyed|merged)"(?:,\s*"[a-z_]+")*)\]\.includes\(',
                             FRONTEND_SOURCE)
        self.assertTrue(filters, "找不到前端的退場狀態清單")
        for raw in filters:
            recognised = set(re.findall(r'"([a-z_]+)"', raw))
            for status in written - {"active"}:
                self.assertIn(status, recognised,
                              f"前端有一份退場清單不認得後端寫的 {status}：{raw}")
        self.assertNotIn('"merged"', FRONTEND_SOURCE,
                         "前端還留著沒人讀的 merged 狀態")
        self.assertNotIn('"merged"', engine_source,
                         "後端還留著沒人讀的 merged 狀態")

    def test_the_merge_retires_the_absorbed_army_for_real(self):
        engine = GameEngine(seed=5)
        engine.new_game()
        engine._tactical = {
            "armies": {
                "C-1": {"generalId": "liu_xiang", "faction": "C", "status": "active",
                        "units": {"infantry": 8, "cavalry": 2,
                                  "machine_gun": 1, "artillery": 1}},
                "Q-1": {"generalId": "qian_local_militia", "faction": "Q",
                        "status": "active",
                        "units": {"infantry": 8, "cavalry": 2,
                                  "machine_gun": 1, "artillery": 1}},
            },
        ...
        card = next(c for c in json.loads(
            EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
            if c["id"] == "liu_xiang_annexes_qian")
        applied = engine._apply_event_payload(card["apply"],
                                              players=list(engine.state["players"]),
                                              card=card)
        entry = next(a for a in applied if a["kind"] == "npc_faction_merge")
        qian = engine._tactical["armies"]["Q-1"]
        self.assertIn(qian["status"], GameEngine.DEAD_ARMY_STATUSES)
        self.assertEqual(GameEngine._force_of(qian["units"]), 0)
        # 這支部隊在後端的「還活著的 NPC 部隊」名單裡也必須消失。
...
class NpcVisibleStateTests(unittest.TestCase):
    ...
    def test_city_levels_come_from_the_override_table(self):
        ...
        # 權威從 city_level_overrides 換成了 city_output——後者是後端拿覆寫表
        # 算完的**等級＋產出**，全圖每一座城都有。要守的仍然是同一件事：
        # 不准只讀 city_economy（那張表只有玩家自己的城）。
        self.assertIn("city_output", stripped,
                      "城市等級沒有讀後端算好的那一份，NPC 手上的城會同步不到")
...
class NewGameAndProvinceNamingTests(unittest.TestCase):
    """第二十三批：使用者回報「開了新局，除了駐軍以外地格還是沒變」。

    兩個獨立的病根，都是**一份在權威計算之前拍的快照／清單蓋掉了正確答案**：

      * `INITIAL_CELL_FACTIONS` 是 module load 時抓的，而
        `applyProvinceOwnershipClaims()` 是 `boot()` 裡才跑的。resetGame()
        把 cell.fac 寫回那份快照，等於把省級歸屬保證整套抹掉。
      * `strategicProvinceForCell()` 只認「有城市的省份」。青海整省沒有城
        （西寧照舊民國地圖歸甘肅），於是幾何答對了「青海」也不算數，
        一路掉到「最近的城市是哪一省」→ 西寧 → 甘肅。
    """
    ...
    def test_the_stale_snapshot_name_is_gone(self):
        ...
        self.assertNotIn("INITIAL_CELL_FACTIONS", self._app_js())

    def test_province_naming_trusts_the_border_file_not_the_city_list(self):
        body = re.search(r"function strategicProvinceForCell\(cell\) \{(.*?)\n\}",
                         self._app_js(), re.S).group(1)
        stripped = self._no_comments(body)
        self.assertNotIn("provinceOptions()", stripped,
                         "又拿「有沒有城市」當省份存在與否的判準了")
        self.assertIn("mapProvinceNames()", stripped)

    def test_qinghai_really_has_no_city(self):
        """上面那條要有意義，前提是青海真的一座城都沒有。"""
        cities = load_game_data()["strategic_map"]["cities"]
        provinces = {c.get("province") for c in cities}
        self.assertNotIn("青海", provinces,
                         "青海有城市了？那這一組驗證就空轉了，改挑別的省")
        self.assertIn("甘肅", provinces)

    def test_the_border_file_puts_qinghai_and_xining_where_we_decided(self):
        """幾何本身要對：青海湖在青海、西寧仍在甘肅。"""
        from backend.data_store import REPO_ROOT
        geojson = json.loads(
            (REPO_ROOT / "frontend/data/provinces_1926.geojson").read_text(encoding="utf-8"))
        ...
        def rings(feature):
            ...
            return [coords] if feature["geometry"]["type"] == "Polygon" else coords
        ...
        def province_at(lon, lat):
            for feature in geojson["features"]:
                for polygon in rings(feature):
                    if polygon and polygon[0] and inside(lon, lat, polygon[0]):
                        return feature["properties"]["name"]
            return None

        self.assertEqual(province_at(100.2, 36.9), "青海", "青海湖")
        self.assertEqual(province_at(98.1, 36.3), "青海", "都蘭")
        self.assertEqual(province_at(101.8, 36.6), "甘肅", "西寧")
        self.assertEqual(province_at(103.8, 36.06), "甘肅", "蘭州")


class OutputAndRuleOwnershipTests(unittest.TestCase):
    """第二十四批：產能有沒有跟著等級走，以及規則有沒有偷偷住在前端。

      * 城市升級之後**產能要跟上**。等級讀 `city_level_overrides`（全圖都有）、
        產出卻只讀 `city_economy`（只有玩家的城），於是 NPC 手上的城
        升級之後標籤寫「3 級城市」、產出還印著 2 級的數字。
      * 將領直屬名額（大將 3、中將 2、少將 0、中將上限 3）先前只寫在 app.js，
        後端連〈擴編直屬〉的上限都無從把關。
    """
    ...
    # ── 產能跟著等級走 ──────────────────────────────────────────────
    def test_output_follows_the_level_for_every_city(self):
        engine = GameEngine(seed=5)
        before = engine.snapshot()["city_output"]
        self.assertIn("zunyi", before, "全圖每一座城都要在裡面，不只玩家的")
        self.assertEqual(before["zunyi"], {"level": 2, "cash": 2, "factory": 2})
        engine.state.setdefault("city_level_overrides", {})["zunyi"] = 3
        after = engine.snapshot()["city_output"]["zunyi"]
        self.assertEqual(after["level"], 3)
        self.assertGreater(after["cash"], before["zunyi"]["cash"],
                           "等級升了產能沒跟上")
        self.assertGreater(after["factory"], before["zunyi"]["factory"])

    def test_the_report_covers_cities_no_player_owns(self):
        """這條要有意義，前提是遵義真的不在任何玩家的 city_economy 裡。"""
        engine = GameEngine(seed=5)
        listed = {city["id"] for code in engine.state["players"]
                  for city in engine._city_economy_for(code)}
        self.assertNotIn("zunyi", listed)
        self.assertIn("zunyi", engine.snapshot()["city_output"])

    def test_the_card_actually_moves_both_numbers(self):
        """〈黔軍整頓茅台酒造〉：真的跑一次卡，等級與產能一起動。"""
        engine = GameEngine(seed=5)
        card = next(c for c in json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
                    if c["id"] == "qian_army_moutai")
        before = engine.snapshot()["city_output"]["zunyi"]
        engine._apply_event_payload(card["apply"],
                                    players=list(engine.state["players"]), card=card)
        after = engine.snapshot()["city_output"]["zunyi"]
        self.assertEqual(after["level"], before["level"] + 1)
        self.assertEqual(after["cash"], before["cash"] + 1)
        self.assertEqual(after["factory"], before["factory"] + 1)

    def test_the_frontend_reads_the_resolved_output(self):
        body = re.search(r"function syncStrategicCitiesFromState\(\) \{(.*?)\n\}",
                         self._app_js(), re.S).group(1)
        stripped = self._no_comments(body)
        self.assertIn("state.city_output", stripped,
                      "產出又只讀玩家的 city_economy 了，NPC 的城會等級與產能各說各話")

    # ── 規則不准只住在前端 ──────────────────────────────────────────
    def test_general_slot_rules_live_in_the_backend(self):
        from backend.card_engine import FEATURES
        slots = FEATURES["general_slots"]
        self.assertEqual(slots["great_general"], 3)
        self.assertEqual(slots["lieutenant_general"], 2)
        self.assertEqual(slots["major_general"], 0)
        self.assertEqual(slots["lieutenant_general_cap"], 3)

    def test_the_frontend_no_longer_hard_codes_them(self):
        stripped = self._no_comments(self._app_js())
        self.assertNotIn("const LIEUTENANT_SLOT_CAP", stripped)
        self.assertNotIn("const GENERAL_SLOT_DEFAULTS", stripped)
        self.assertIn('backendRule("features.general_slots")', stripped)

    def test_the_backend_enforces_the_slot_cap(self):
        engine = GameEngine(seed=5)
        player = sorted(engine.state["players"])[0]
        card_id = next(c["id"] for c in engine.data["function_cards"]["cards"]
                       if c.get("mechanic") == "affiliation_slot")
        engine.state["players"][player]["hand"] = [card_id]
        with self.assertRaises(ValueError):
            engine.use_function(player, card_id,
                                target_general_id="he_yingqin", current_slots=3)
        # 還沒滿的時候要放行。
        engine.state["players"][player]["hand"] = [card_id]
        result = engine.use_function(player, card_id,
                                     target_general_id="he_yingqin", current_slots=2)
        self.assertEqual(result["affiliation_slot_delta"]["cap"], 3)

    def test_every_function_card_mechanic_is_known_to_the_backend(self):
        """前端不准有後端不認得的機制——那就是「只在畫面上發生」。"""
        from backend.data_store import REPO_ROOT
        source = (REPO_ROOT / "backend/card_engine.py").read_text(encoding="utf-8")
        handled = set(re.findall(r'mechanic == "([a-z_]+)"', source))
        for group in re.findall(r"mechanic in \(([^)]*)\)", source):
            handled |= {piece.strip().strip("\"'") for piece in group.split(",") if piece.strip()}
        cards = load_game_data()["function_cards"]["cards"]
        unknown = sorted({c["mechanic"] for c in cards
                          if c.get("mechanic") and c["mechanic"] not in handled})
        self.assertEqual(unknown, [], f"這些機制後端沒有分派：{unknown}")

    # ── 削兵的規則只有一份 ──────────────────────────────────────────
    def test_cutting_to_a_force_target_does_not_overshoot(self):
        """「裁到 N 點」要命中 N，不能一刀砍過頭。"""
        engine = GameEngine(seed=5)
        units = {"infantry": 15, "cavalry": 2, "artillery": 4, "machine_gun": 2}
        result = engine.cut_units_to_force(units, multiplier=0.6)
        self.assertEqual(result["force_before"], 37)
        self.assertEqual(result["target"], 22)
        self.assertEqual(result["force_after"], 22,
                         "裁過頭了——「−40%」會變成更大的損失")

    def test_the_naive_cut_really_would_overshoot(self):
        """這條在於證明上面那關**驗得到東西**：天真的裁法真的會砍過頭。"""
        from backend.card_engine import UNIT_FORCE_POINTS
        units = {"infantry": 15, "cavalry": 2, "artillery": 4, "machine_gun": 2}
        target = 22
        naive = dict(units)
        order = sorted(UNIT_FORCE_POINTS, key=lambda u: -UNIT_FORCE_POINTS[u])
        force = lambda d: sum(d.get(u, 0) * p for u, p in UNIT_FORCE_POINTS.items())
        while force(naive) > target:
            unit = next((u for u in order if naive.get(u, 0) > 0), None)
            if unit is None:
                break
            naive[unit] -= 1
        self.assertLess(force(naive), target,
                        "天真的裁法沒有砍過頭？那這組數字挑得不好，換一組")

    def test_a_multiplier_of_one_changes_nothing(self):
        engine = GameEngine(seed=5)
        units = {"infantry": 15, "cavalry": 2, "artillery": 4, "machine_gun": 2}
        result = engine.cut_units_to_force(units, multiplier=1.0)
        self.assertEqual(result["units"], {**units})
        self.assertEqual(result["force_after"], result["force_before"])

    def test_a_negative_multiplier_is_a_loud_error(self):
        engine = GameEngine(seed=5)
        with self.assertRaises(ValueError):
            engine.cut_units_to_force({"infantry": 5}, multiplier=-0.5)

    def test_the_frontend_asks_the_backend_to_cut(self):
        stripped = self._no_comments(self._app_js())
        body = re.search(r"async function scaleArmyForce\(army, multiplier\) \{(.*?)\n\}",
                         self._app_js(), re.S).group(1)
        self.assertIn('api("/api/cut-force"', self._no_comments(body),
                      "前端又自己裁兵了——同一條規則會有兩個答案")
        baseline = re.search(
            r"async function setArmyForceFromBaseline\(army, chain, remaining\) \{(.*?)\n\}",
            self._app_js(), re.S).group(1)
        self.assertIn('api("/api/cut-force"', self._no_comments(baseline))
        self.assertIn("/api/cut-force", stripped)

    def test_the_route_is_wired_up(self):
        from backend.data_store import REPO_ROOT
        server = (REPO_ROOT / "backend/server.py").read_text(encoding="utf-8")
        self.assertIn('"/api/cut-force": self._cut_force', server)

    def test_trait_names_do_not_drift_between_the_two_copies(self):
        """技能中文名在 general_traits.json 與 app.js 各有一份。

        目前兩份一致；這條測試在於它們**開始不一致的那一天**要有人叫。
        """
        traits = load_game_data()["general_traits"]["traits"]
        block = re.search(r"const TRAIT_LABELS = \{(.*?)\n\};",
                          self._app_js(), re.S).group(1)
        front = dict(re.findall(r'^\s*([a-z_0-9]+):\s*"([^"]*)"', block, re.M))
        self.assertEqual(sorted(front), sorted(traits), "兩份技能清單的項目對不上")
        drifted = {key: (front[key], traits[key].get("name"))
                   for key in front if front[key] != traits[key].get("name")}
        self.assertEqual(drifted, {}, f"技能中文名兩邊對不上：{drifted}")


class RevivalUnlockAndImmunityTests(unittest.TestCase):
    """第二十五批：兵力復活、事件卡解鎖的功能卡、以及免疫／封鎖類。

    抓到的兩個缺陷都是老家族：

      * 12.49〈國際紅十字救護團〉卡面寫「復原隨機 **2** 個營」，資料檔也寫了
        `units: 2`——但那個欄位**沒有人讀**，實際只補 1 營。
      * 綁省份的免疫（10.8 復興儒學）後端擋得住，但畫面上的「指定將領」
        照樣把免疫的目標列出來，玩家選了按了才收到「本牌對其無效」。
    """
    ...
    @staticmethod
    def _draw(engine, card_id, limit=14):
        engine.state["event_pool"] = [card_id]
        view = None
        for _ in range(limit):
            engine.next_turn(active_player=sorted(engine.state["players"])[0])
            view = engine.pending_event_view()
            if view:
                break
        applied, guard = [], 0
    ...
    def _open(seed=11):
        ...
        for payload in engine.state["players"].values():
            ...
            payload["factory_points"] = 300
            for power in payload.get("foreign_relations", {}):
                payload["foreign_relations"][power] = 0
        return engine

    # ── 兵力復活 ────────────────────────────────────────────────────
    def test_the_red_cross_really_heals_two_battalions(self):
        """12.49 卡面寫 2 營，資料寫了 units: 2——那個欄位要真的有人讀。"""
        engine = GameEngine(seed=5)
        player = sorted(engine.state["players"])[0]
        engine.state["players"][player]["timed_effects"] = [
            {"kind": "field_hospital_window", "remaining_turns": 3, "units": 2}]
        self.assertEqual(engine._field_hospital_battalions(player, "anyone"), 2)

    def test_the_swimmer_card_still_heals_one(self):
        """5.2 沒寫 units，吃預設值 1。"""
        engine = GameEngine(seed=5)
        player = sorted(engine.state["players"])[0]
        engine.state["players"][player]["timed_effects"] = [
            {"kind": "field_hospital_window", "remaining_turns": 3}]
        self.assertEqual(engine._field_hospital_battalions(player, "anyone"), 1)

    def test_penicillin_uses_the_card_number(self):
        engine = GameEngine(seed=5)
        player = sorted(engine.state["players"])[0]
        engine.state["players"][player]["timed_effects"] = []
        engine.state["players"][player]["field_hospital_generals"] = ["he_yingqin"]
        card = next(c for c in engine.data["function_cards"]["cards"]
                    if c.get("mechanic") == "field_hospital")
        self.assertEqual(engine._field_hospital_battalions(player, "he_yingqin"),
                         int(card["recover_battalions"]))

    def test_the_better_source_wins_when_both_apply(self):
        engine = GameEngine(seed=5)
        player = sorted(engine.state["players"])[0]
        engine.state["players"][player]["timed_effects"] = [
            {"kind": "field_hospital_window", "remaining_turns": 3, "units": 2}]
        engine.state["players"][player]["field_hospital_generals"] = ["he_yingqin"]
        self.assertEqual(engine._field_hospital_battalions(player, "he_yingqin"), 2)

    def test_the_card_data_still_says_two(self):
        """上面那條要有意義，前提是 12.49 真的寫著 units: 2。"""
        cards = {c.get("ref"): c for c in
                 json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]}
        flags = cards["12.49"]["apply"]["timed_flags"]
        self.assertEqual(flags[0]["units"], 2)
        self.assertNotIn("units", cards["5.2"]["apply"]["timed_flags"][0])

    def test_the_recovery_report_says_how_many(self):
        """畫面先前一律寫「+1 營」，補了兩營也寫 1。"""
        body = re.search(r"function applyFieldHospitalResult\(report\) \{(.*?)\n\}",
                         self._app_js(), re.S).group(1)
        stripped = self._no_comments(body)
        self.assertIn("entry.picked", stripped,
                      "畫面沒有讀後端送的實際補兵清單")
        # 只看這支函式：別處的「各 +1 營」是 NPC 例行補兵，那裡本來就是 1。
        self.assertNotIn("+1 營", stripped)

    # ── 事件卡解鎖的功能卡 ──────────────────────────────────────────
    UNLOCK_CHAIN = (("3.1", "event_sound_film", "sound_film_studio"),
                    ("3.2", "event_radio_network", "state_radio_station"),
                    ("6.2", "event_model_a_car", "mechanized_division"),
                    ("7.1", "event_government_scholars", "government_scholars"),
                    ("7.4", "event_penicillin", "penicillin_import"),
                    ("8.4", "event_graf_zeppelin", "zeppelin_recon"))

    def test_every_unlock_flag_has_a_card_that_wants_it(self):
        """解鎖旗標開了卻沒有卡在等它＝空轉。"""
        cards = json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
        granted = set()
        ...
        def walk(node):
            if isinstance(node, dict):
                for key, value in node.items():
                    if key == "unlock":
                        granted.update(value if isinstance(value, list) else [value])
                    walk(value)
            elif isinstance(node, list):
                for value in node:
                    walk(value)
        for card in cards:
            walk(card.get("apply") or {})
        wanted = {c.get("requires_unlock") for c in load_game_data()["function_cards"]["cards"]
                  if c.get("requires_unlock")}
        # event_goddard_rocket 沒有功能卡在等它——它是戰鬥修正的開關，
        # combat_modifiers.situational_modifiers 會讀。這是刻意的例外。
        orphan = sorted(granted - wanted - {"event_goddard_rocket"})
        self.assertEqual(orphan, [], f"這些解鎖旗標沒有任何卡在等：{orphan}")

    def test_the_locked_card_cannot_be_played_before_the_event(self):
        for ref, unlock, card_id in self.UNLOCK_CHAIN:
            engine = self._open()
            player = sorted(engine.state["players"])[0]
            engine.state["players"][player]["hand"] = [card_id]
            with self.assertRaises(ValueError, msg=f"{ref} {card_id}"):
                engine.use_function(player, card_id)

    def test_the_event_grants_the_flag_and_shuffles_the_card_in(self):
        for ref, unlock, card_id in self.UNLOCK_CHAIN:
            engine = self._open()
            player = sorted(engine.state["players"])[0]
            before = engine._card_count_in_player_zones(engine._player(player), card_id)
            card = next(c for c in json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]
                        if c.get("ref") == ref)
            self._draw(engine, card["id"])
            self.assertIn(unlock, engine._player(player).get("unlocks") or [],
                          f"{ref} 沒有給出 {unlock}")
            after = engine._card_count_in_player_zones(engine._player(player), card_id)
            self.assertGreater(after, before, f"{ref} 解鎖了但 {card_id} 沒進牌庫")

    # ── 免疫 ────────────────────────────────────────────────────────
    def test_province_immunity_blocks_the_card_and_says_so(self):
        engine = self._open()
        self._draw(engine, "confucian_revival")
        holder = sorted(engine.state["players"])[0]
        self.assertIsNotNone(
            engine.province_card_immunity(holder, "local_autonomy_agitation"))
        published = engine.snapshot()["immune_targets"]
        self.assertIn("local_autonomy_agitation", published)
        self.assertIn(holder, published["local_autonomy_agitation"])
        self.assertIn("無效", published["local_autonomy_agitation"][holder]["note"])

    def test_the_target_list_marks_immune_factions(self):
        stripped = self._no_comments(self._app_js())
        self.assertIn("immuneTarget(card.id, owner)", stripped,
                      "指定將領的下拉又把免疫的目標當成可選了")
        self.assertIn("immune_targets", stripped)

    def test_widening_and_suspending_run_through_the_real_draw(self):
        """10.1 擴大、10.3 按下——兩張都走 conditional_branch 的 if_active。"""
        cards = {c.get("ref"): c for c in
                 json.loads(EVENT_CARDS_PATH.read_text(encoding="utf-8"))["cards"]}
        engine = self._open()
        self._draw(engine, "confucian_revival")
        applied = self._draw(engine, cards["10.1"]["id"])
        branch = next(x for x in applied if x.get("kind") == "conditional_branch")
        self.assertEqual(branch["chosen"], "if_active", branch)
        holder = sorted(engine.state["players"])[0]
        provinces = (engine._player(holder)["province_card_immunities"][0]["provinces"])
        self.assertIn("直隸", provinces)

        engine2 = self._open()
        self._draw(engine2, "confucian_revival")
        applied2 = self._draw(engine2, cards["10.3"]["id"])
        branch2 = next(x for x in applied2 if x.get("kind") == "conditional_branch")
        self.assertEqual(branch2["chosen"], "if_active", branch2)
        entry = engine2._player(holder)["province_card_immunities"][0]
        self.assertIsNotNone(entry.get("suspended_until_turn"))
        self.assertIsNone(
            engine2.province_card_immunity(holder, "local_autonomy_agitation"),
            "免疫被按下去了，province_card_immunity 卻還說有效")
