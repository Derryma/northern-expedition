...
class GameEngine:
    ...
    def academia_active(self, player: str) -> bool:
        ...
        if not status.get("holder") or status.get("disqualified"):
            return False
    ...
    def _unit_cost_for(self, player: str, unit_type: str) -> tuple[int, int]:
        ...
        cash = max(1, cash + int(adjustment.get("cash", 0)) + timed["cash"])
        factory = max(0, int(base["factory"]) + int(adjustment.get("factory", 0)) + timed["factory"])
        return cash, factory
    ...
    def _start_event_cycle(self) -> bool:
        ...
        for index in range(count):
            ...
            eligible = [
                ...
                if self._event_eligible_players(self._event_template(card_id))
            ]
            if not eligible:
                break
        ...
        for code in targets:
            ...
            for power, amount in deltas.items():
                if power not in relations:
                    continue
                before = int(relations[power])
                relations[power] = max(FOREIGN_RELATION_MIN, min(FOREIGN_RELATION_MAX, before + int(amount)))
        ...
        for flag in payload.get("timed_flags") or []:
            for code in targets:
                entry = {
                    ...
                    "kind": flag["kind"],
                    "remaining_turns": int(flag.get("turns", 1)),
                    "owners": [code],
                }
    ...
    def _sync_suspendable_cards(self, player: str) -> None:
        ...
        if self.academia_status(player).get("holder"):
    ...
    def _controls_city_of_level(self, player: str, level_min: int) -> bool:
        ...
        for city in self.data["strategic_map"]["cities"]:
            if int(city.get("level", 0)) < level_min:
                continue
            if self.state["city_owners"].get(city["id"], city["faction"]) == player:
                return True
        return False
    ...
    def _card_template(self, card_id: str, player: Optional[str] = None) -> Dict[str, Any]:
        ...
        for override in (self.state or {}).get("function_card_overrides", []) if getattr(self, "state", None) else []:
