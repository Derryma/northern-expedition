...
`data/function_cards.json` 目前收錄 119 張卡。只有下面列出的會實際進入牌庫，其餘是從原始素材沿用下來的目錄項目，沒有 `mechanic` 欄位，引擎會拒絕打出。
...
| 奉系（F） | 8 | −7 | 4 | 1 | 2 | 147 |
| 直系（W） | −3 | −8 | 6 | 2 | 5 | 144 |
| 五省聯軍（S） | 3 | −6 | 5 | 3 | 4 | 147 |
| 國民革命軍（N） | −1 | 9 | −8 | −4 | 1 | 154 |
...
| 對日／對蘇／對英／對法／對美貿易出口 | 各 3 | 消耗 50 工業點，現金 +20、該國關係 +1。工業點不足就打不出來，且失敗不扣點。 |
...
`story` 純粹是敘事，**不影響任何判定**；規則一律寫在 `effect` 裡。目前 27 張現役卡帶有故事，
