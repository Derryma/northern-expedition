...
名冊上加城市（第二十六批）：一次加了六座——信陽（河南・直系）、柳州（廣西・
國民革命軍）、赤峰（熱河・奉系）、惠州（廣東・國民革命軍）、沙市（湖北・直系）
都是二級，徐州（江蘇・五省聯軍）是三級。**只動 `scenario/data/strategic_map.json`
一個檔**——等級、產出、地格落點、鐵路沿線名單全部是既有機制自己算出來的，
沒有任何一處要為新城市寫特例。

加城市之前先量、加完再量一次，比對的是**每座城市的 `cellKey`**：
`indexScenarioCells()` 是照名冊順序一座一座搶格子的，插在中間的城市有可能
把後面那座擠走。這一輪實測既有 68 座**一座都沒動**，各家格數也一格沒變
（M69/D57/C64/G140/Q26/N72/W66/H25/Y35/F273/S81，前後相同）。
量法：無頭開一次真前端，把 `bootstrap.strategic_map.cities` 的 `cellKey`
與 `cells[].fac` 印出來存成 JSON，改動前後 diff。**加城市一律先做這件事。**

三件順帶要知道的事：

* **信陽與徐州自動變成車站。** `cities_along_railways()` 是拿城市座標對鐵路
  折線量距離的（`RAILWAY_STATION_DEGREES = 0.9`），而京漢鐵路的折線本來就有
  一個點在 `[114.1, 32.1]`（信陽），津浦與隴海則同時經過 `[117.2, 34.3]`
  （徐州）。所以 12.58 那類「沿線每座城市」的卡片從現在起會多算它們。
  這是對的，但要知道它會動到卡片的作用範圍。
* **徐州釘了 `cell_key = "29,22"`。** 那是附近唯一同時掛著津浦與隴海的格子，
  交會點比格子的省界標籤重要。副作用：那一格報「安徽」而城市自己報「江蘇」。
  大同（山西／察哈爾）早就是同一種情形，卡片選城市看的是城市自己的
  `province`，所以不影響卡片作用範圍——但地格資訊面板上會看到這個落差。
* **產出不是在 JSON 裡寫死的。** `economy/output.py` 的 `scaled_city_value()`
  早就不讀名冊裡那組 `cash`/`factory` 了，**等級是唯一的輸入**。名冊裡照樣
  填那兩個欄位只是為了跟舊資料一致，改它們一點用都沒有。守門測試就是拿
  同級城市逐項比對來釘死這件事。

守門在 `CityRosterTests`（id 不撞號、六座都在名冊上且等級正確、陣營與同省
其他城市一致、產出等於同級城市、信陽在京漢沿線、徐州同時在津浦與隴海沿線、
另外四座不在任何鐵路上、徐州釘在交會格）。落點、顏色與「有沒有擠開別人」
由 `province_ownership_e2e.py` 既有的三關驗（釘選的城市都在釘住的那一格／
沒釘的沒被擠開超過一格／每座城市都站在自家顏色的格上），這一輪 32 關全過。

**這一種改動舊存檔看得到**——與上面那條存檔警告不同，別搞混：城市名冊走
bootstrap 每次都重送，而 `applyTacticalSnapshot()` 對 `cityFactions` 是
**只覆寫存檔裡有的那幾座**（`if (snapshot.cityFactions?.[city.id])`），
名冊上新加的城市不在存檔裡，所以保留劇本歸屬。實測：拿 repo 裡那份
`game_data/tactical_state.json` 開一次頁面，六座全部上圖、`cell.fac`
與城市陣營一致。會被存檔蓋掉的是**地格顏色與部隊位置**（也就是改 `map.js`／
省界／駐防的那一類），不是名冊本身。

再加三座與兩個港口（第二十七批）：榆林（陝西・西北軍）、咸陽（陝西・西北軍）、
自貢（四川・川軍）都是二級；同時把惠州改成**海港**、沙市改成**河港**。
名冊現在 77 座、36 座港市。既有城市**一座都沒被擠開**（量法同上一批）。

（自貢一度也做成河港，使用者當場改回普通陸地城市。回退要**兩邊一起改**：
名冊拿掉 `"port"`、`RIVER_PORTS` 撤掉 id。只回退一半的話，`_select_cities`
是先看 waters 再看 port，會出現「不是港口卻被長江水患挑到」。
守門在 `test_zigong_is_a_plain_inland_city` 與 e2e 的三關，
突變體 P7／P7b 就是分別咬這兩半的。）

**這一批挖到一個真缺陷，跟新城市無關，是舊的：成都與襄陽是「地圖上的長江
河港、後端不屬於任何水系」。** 水域名單
（`backend/foreign_punishment.RIVER_PORTS`）是手維護的，這兩座從來沒被寫進去，
`waters_for_city()` 對它們回傳空陣列——長江水患／封鎖挑港市時挑不到。可是前端
`markRiverPortWater()` 另外拿 `nearestRiverName()` 量最近的河，把它們標成
「河港・長江」，`blockadedWaters()[cell.river]` 連封鎖的斜紋都照塗。
**畫面說被淹、結算說沒事**——又是「同一條規則前後端各寫一份」。

修法照老規矩，規則只留一份在後端：

* `_strategic_map_snapshot()` 對每座港市送出 `city["waters"]`
  （河港查 `RIVER_PORTS`，海港走 `coastal_sea_name()`，與封鎖引擎同一份）；
* 前端 `portWaterName()` **只讀**；名單漏掉時**大聲丟例外**，不再悄悄補一個河名；
* `nearestRiverName()` 與 `pointSegmentDistance()` 直接刪掉（前端已無人使用）；
* 支流一律掛幹流：漢水（襄陽）、岷江（成都）都歸長江。

**加港口城市的檢查清單**（照做，不要憑印象）：

1. 名冊加 `"port": "river"` 或 `"sea"`；
2. 河港**一定要**在 `RIVER_PORTS` 裡選一條幹流。海港不用；
3. 開真前端量三件事——地格有沒有變成水域、河名等不等於後端給的水系、
   鄰格有沒有同一條河（沒有就是一座艦隊到不了的孤港）；
4. 海港的地格必須至少貼著一格近海。

實測結果：沙市的地格接上長江幹流（三格鄰居都是長江），惠州貼著兩格近海。
兩座陸軍都走得過去、艦隊都進得去。

**通行規則沒有被動到，但要知道它長怎樣**：河港的地格是水域，
`riverStepAllowed()` 對「城市是河港」的格子直接放行（不必架浮橋），
所以沙市不會把陸軍卡在自己家門口；海港的地格仍是陸地，走法照舊。

守門在 `PortWaterRosterTests`（每座港市都問得出水域、每座河港剛好落在一份
名單裡、名單裡沒有陌生 id、海港算得出海域、bootstrap 真的把 waters 送出去、
長江水患挑得到沙市、惠州算進南海的港市數、自貢兩邊都乾淨）與
`scripts/checks/port_terrain_e2e.py`（真前端 23 關）＋
`scripts/checks/mutate_port_terrain.py`（13 個突變體，全抓）。

**兩個做法上的教訓：**

* **對照組不能省。** 「河港放陸軍通行」單獨看是假綠——`riverStepAllowed()`
  整條 `return true` 也會過。e2e 另外掃全圖沒有城市的河道格，斷言它們照樣
  攔得住陸軍；突變體 P9 就是靠那一關抓到的。
* **等價突變體要補路徑，不是刪突變體。** `portWaterName()` 的 throw 在資料
  完整時永遠跑不到，改成「回傳內河」與原始碼等價。解法是在 e2e 裡直接拿一座
  假的港市去問它、斷言拋例外。補完之後 11/11 全抓。

順帶釘住的一件事：**咸陽算隴海鐵路的車站**（它就貼在隴海西端的西安旁邊，
`RAILWAY_STATION_DEGREES = 0.9` 量得到）。這是對的，測試把它寫死免得日後
